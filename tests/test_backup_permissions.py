...
class TestBackupPermissions:
    ...
    def test_viewer_cannot_create_backup(self, viewer_headers):
        ...
        response = requests.post(
            ...
            json={
                "backup_name": "unauthorized_backup",
        ...
        if response.status_code == 403:
            ...
            if error_detail:
                ...
                assert 'permission' in error_detail_lower or 'forbidden' in error_detail_lower or 'admin' in error_detail_lower, \
    ...
    def test_viewer_cannot_restore_backup(self, viewer_headers):
        ...
        assert response.status_code not in [200, 201], \
            f"SECURITY VIOLATION: Viewer was able to restore backup!"
    ...
    def test_viewer_cannot_run_retention_cleanup(self, viewer_headers):
        ...
        response = requests.post(
            f"{BASE_URL}/api/backup/retention/run",
            headers=viewer_headers,
            verify=VERIFY_SSL
        )
    ...
    def test_viewer_cannot_delete_backup(self, viewer_headers):
        ...
        response = requests.delete(
            f"{BASE_URL}/api/backup/delete/test_backup.zip",
            headers=viewer_headers,
            verify=VERIFY_SSL
        )
    ...
    def test_admin_can_create_backup(self, admin_headers):
        ...
        if response.status_code == 500:
            assert "Too many concurrent" in response.text, \
                f"500 should be rate limiting: {response.text}"
        else:
    ...
    def test_admin_can_list_backups(self, admin_headers):
        ...
        assert response.status_code == 200, \
            f"Admin should list backups. Got {response.status_code}: {response.text}"
    ...
    def test_admin_can_run_retention_cleanup(self, admin_headers):
        ...
        if response.status_code == 500:
            ...
            assert any(msg in response_text for msg in ["too many concurrent", "no security policy"]), \
