...
class TestDirectPrivilegeEscalation:
    ...
    def test_cannot_modify_own_user_record(self, escalation_tester):
        ...
        response = escalation_tester.put(
            ...
            {"email": "admin@pwned.com", "roles": ["admin"], "is_active": True},
        )

        assert response.status_code == 403, f"CRITICAL: Guest modified own user record! Status: {response.status_code}"
    ...
    def test_cannot_create_admin_user(self, escalation_tester):
        ...
        """
        response = escalation_tester.post(
            "/api/users/register",
            {
        ...
        if roles_response.status_code == 200:
            ...
            if viewer_role_id:
                ...
                response = escalation_tester.put(
                    ...
                    {"permissions": ["*:*", "admin", "user:create", "user:update", "user:delete"]},  # All permissions
                )
            ...
            for user in users_data.get("data", {}).get("users", []):
                if "admin" in [r.get("name") for r in user.get("roles", [])]:
                    ...
                    deactivate_response = escalation_tester.put(
                        f"/api/users/{admin_user_id}/status", {"is_active": False}
                    )

                    assert (
                        deactivate_response.status_code == 403
        ...
        if response.status_code == 200:
            ...
            for role in roles_data.get("data", []):
                if role.get("name") == "admin":
                    ...
                    delete_response = escalation_tester.delete(f"/api/users/roles/{admin_role_id}")
...
class TestInformationDisclosure:
    ...
    def test_enumerate_all_users(self, escalation_tester):
        ...
        assert response.status_code == 403, f"INFO LEAK: Guest enumerated all users! Status: {response.status_code}"
    ...
    def test_view_specific_user_details(self, escalation_tester):
        ...
        assert response.status_code == 403, f"INFO LEAK: Guest viewed user details! Status: {response.status_code}"
...
class TestChainedEscalation:
    ...
    def test_full_escalation_chain(self, escalation_tester):
        ...
        step1 = escalation_tester.post(
            "/api/users/roles/add_role", {"name": "backdoor_admin", "permissions": ["*:*", "admin"]}
        )
        ...
        if step1.status_code == 200:
            ...
            step2 = escalation_tester.put(
                f"/api/users/{escalation_tester.guest_user_id}/roles", {"role_ids": [role_id]}
            )
        ...
        assert step1.status_code == 403, f"First step of escalation chain succeeded! Status: {step1.status_code}"
