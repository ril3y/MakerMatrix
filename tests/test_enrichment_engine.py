...
from MakerMatrix.suppliers.base import PartSearchResult, EnrichmentResult, SupplierCapability, SupplierInfo
...
class TestEnrichmentEngine:
    ...
    async def test_engine_uses_scraping_when_no_credentials(self):
        ...
        mock_supplier = MockSupplier(name="test-scraper", supports_scraping=True, has_credentials=False)

        engine = EnrichmentEngine()

        with patch("MakerMatrix.services.system.enrichment_engine.get_supplier", return_value=mock_supplier):
            result = await engine.enrich_part(supplier_name="test-scraper", part_identifier="TEST-123")
    ...
    async def test_engine_uses_api_when_credentials_available(self):
        ...
        mock_supplier = MockSupplier(name="test-api", supports_scraping=False, has_credentials=True)

        engine = EnrichmentEngine()

        with patch("MakerMatrix.services.system.enrichment_engine.get_supplier", return_value=mock_supplier):
            result = await engine.enrich_part(supplier_name="test-api", part_identifier="TEST-123")
    ...
    async def test_engine_is_supplier_agnostic(self):
        ...
        for supplier_name, supports_scraping, has_creds in suppliers_to_test:
            mock_supplier = MockSupplier(
                name=supplier_name, supports_scraping=supports_scraping, has_credentials=has_creds
            )

            with patch("MakerMatrix.services.system.enrichment_engine.get_supplier", return_value=mock_supplier):
                result = await engine.enrich_part(supplier_name=supplier_name, part_identifier="TEST-123")
    ...
    async def test_engine_delegates_mapping_to_supplier(self):
        ...
        mock_supplier = MockSupplier(name="test-mapper", has_credentials=True)
        ...
        with patch("MakerMatrix.services.system.enrichment_engine.get_supplier", return_value=mock_supplier):
            with patch("MakerMatrix.suppliers.registry.SupplierRegistry.get_supplier", return_value=mock_supplier):
                result = await engine.enrich_part(supplier_name="test-mapper", part_identifier="TEST-123")
    ...
    async def test_engine_handles_enrichment_failure(self):
        ...
        mock_supplier = MockSupplier(name="test-fail", has_credentials=True, api_result=failed_result)

        engine = EnrichmentEngine()

        with patch("MakerMatrix.services.system.enrichment_engine.get_supplier", return_value=mock_supplier):
            result = await engine.enrich_part(supplier_name="test-fail", part_identifier="NONEXISTENT")
    ...
    async def test_engine_handles_missing_supplier(self):
        ...
        with patch("MakerMatrix.services.system.enrichment_engine.get_supplier", return_value=None):
            result = await engine.enrich_part(supplier_name="unknown-supplier", part_identifier="TEST-123")
    ...
    async def test_engine_with_force_refresh(self):
        ...
        async def mock_enrich(supplier_part_number, capabilities=None):
            ...
            return EnrichmentResult(
                ...
                supplier_part_number=supplier_part_number,
                data=PartSearchResult(
                    supplier_part_number=supplier_part_number, manufacturer_part_number=supplier_part_number
                ),
            )
        ...
        with patch("MakerMatrix.services.system.enrichment_engine.get_supplier", return_value=mock_supplier):
            await engine.enrich_part(supplier_name="test", part_identifier="TEST", force_refresh=True)
...
class TestUnifiedEnrichmentPaths:
    ...
    async def test_instant_and_background_use_same_logic(self):
        ...
        engine = EnrichmentEngine()
        with patch("MakerMatrix.services.system.enrichment_engine.get_supplier", return_value=mock_supplier):
            instant_result = await engine.enrich_part(supplier_name="test", part_identifier="TEST-123")

        # Simulate background enrichment call (from part_enrichment_service.py)
        with patch("MakerMatrix.services.system.enrichment_engine.get_supplier", return_value=mock_supplier):
            background_result = await engine.enrich_part(supplier_name="test", part_identifier="TEST-123")
...
class TestSupplierAgnosticDecisionLogic:
    ...
    async def test_no_hardcoded_supplier_names_in_engine(self):
        ...
        forbidden_hardcoded_names = ["mcmaster-carr", "mcmaster_carr", "digikey", "mouser", "lcsc", "adafruit"]
        ...
        for supplier_name in forbidden_hardcoded_names:
            ...
            assert supplier_name.replace("-", "_") not in source_lower or supplier_name in [
                "# ",
                '"""',
                "'''",
    ...
    async def test_decision_based_on_capabilities_only(self):
        ...
        for supports_scraping, has_creds, expected_method in test_matrix:
            mock_supplier = MockSupplier(supports_scraping=supports_scraping, has_credentials=has_creds)

            with patch("MakerMatrix.services.system.enrichment_engine.get_supplier", return_value=mock_supplier):
                result = await engine.enrich_part(supplier_name=f"test-{expected_method}", part_identifier="TEST-123")
