...
def validate_supplier_env_credentials(supplier_name: str, required_fields: list) -> Dict[str, Any]:
    ...
    return {
        ...
        "message": f"Found {len(available_fields)} credentials, missing {len(missing_fields)} required fields" if missing_fields else f"All {len(required_fields)} required credentials available"
    }