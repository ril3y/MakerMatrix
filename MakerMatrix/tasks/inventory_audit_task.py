    ...
    async def execute(self, task: TaskModel) -> Dict[str, Any]:
        ...
        for i, location_id in enumerate(locations_to_audit):
            await self.update_progress(
                task, int(25 + (i / len(locations_to_audit)) * 60), f"Auditing location {i+1}/{len(locations_to_audit)}"
            )
            ...
            audit_results["audit_details"].append({"location_id": location_id, **location_results})
