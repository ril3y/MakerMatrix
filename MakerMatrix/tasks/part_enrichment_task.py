...
class PartEnrichmentTask(BaseTask):
    ...
    async def execute(self, task: TaskModel) -> Dict[str, Any]:
        ...
        try:
            ...
            with Session(engine) as session:
                ...
                enrichment_handlers = EnrichmentCoordinatorService(
                    part_repository, part_service
                )  # Gets CSV config automatically
                ...
                enrichment_result = await enrichment_handlers.handle_part_enrichment(
                    task, progress_callback, session=session
                )
    ...
    async def _update_task_progress(self, task: TaskModel, percentage: int, step: str):
        ...
        if self.task_service:
            try:
                update_request = UpdateTaskRequest(progress_percentage=percentage, current_step=step)
        ...
        try:
            ...
            if total_parts == 0:
                ...
                await self._update_task_progress(
                    task, 100, f"Completed: {successful_count} successful, {failed_count} failed"
                )
            else:
                # All parts succeeded
                await self._update_task_progress(
                    task, 100, f"Bulk enrichment completed successfully - {successful_count} parts enriched"
                )
