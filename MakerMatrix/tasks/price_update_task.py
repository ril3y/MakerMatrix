...
class PriceUpdateTask(BaseTask):
            ...
            for supplier_name, supplier_parts in parts_by_supplier.items():
                ...
                for i, part in enumerate(supplier_parts):
                    try:
                        ...
                        self.log_error(
                            f"Failed to update price for part {part.part_name}: {str(e)}", task, exc_info=True
                        )
                        failed_updates.append(
                            {
                                "part_id": part.id,
    ...
    async def _get_parts_for_update(
        self, session, update_all: bool, part_ids: List[str], supplier_filter: str = None
    ) -> List[PartModel]:
