...
class FileImportEnrichmentTask(BaseTask):
    ...
    async def execute(self, task: TaskModel) -> Dict[str, Any]:
        ...
        if not self.enrichment_handlers:
            with next(get_session()) as session:
                ...
                self.enrichment_handlers = EnrichmentCoordinatorService(
                    part_repository=part_repository, part_service=self.part_service
                )
            ...
            try:
                ...
                if enrichment_result.get("success", False):
                    enriched_parts.append(
                        {"part_id": part_id, "part_name": part_name, "enrichment_result": enrichment_result}
                    )
                    ...
                    self.log_error(
                        f"Failed to enrich part {part_name}: {enrichment_result.get('error', 'Unknown error')}", task
                    )

            except Exception as e:
                self.log_error(f"Failed to enrich part {part_name}: {str(e)}", task, exc_info=True)
                failed_parts.append({"part_id": part_id, "part_name": part_name, "error": str(e)})
        ...
        self.log_info(
            f"File import enrichment complete: {len(enriched_parts)} successful, {len(failed_parts)} failed", task
        )
    ...
    async def _enrich_part_real(self, part_id: str, part_data: Dict[str, Any], task: TaskModel) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log_info(
                f"Starting enrichment for part {part_id} using {enrichment_source} with capabilities: {available_capabilities}",
                task,
            )
            ...
            async def progress_callback(progress, message):
                self.log_info(
                    f"Enrichment progress for {part_data.get('part_name', part_id)}: {progress}% - {message}", task
                )

            # Execute the real part enrichment
            enrichment_result = await self.enrichment_handlers.handle_part_enrichment(
                enrichment_task, progress_callback=progress_callback
            )
