...
class TagModel(SQLModel, table=True):
    ...
    name: str = Field(
        index=True,
        unique=True,
        description="Tag name without # prefix (e.g., 'todo', 'prototype')"
    )
    name_lower: str = Field(
        index=True,
        description="Lowercase version of name for case-insensitive searches"
    )
    ...
    description: Optional[str] = Field(
        default=None,
        description="Optional description of tag purpose/usage"
    )
    icon: Optional[str] = Field(
        default=None,
        max_length=50,
        description="Optional icon name or emoji for UI display"
    )
    ...
    usage_count: int = Field(
        default=0,
        description="Total number of parts and tools using this tag"
    )
    parts_count: int = Field(
        default=0,
        description="Number of parts using this tag"
    )
    tools_count: int = Field(
        default=0,
        description="Number of tools using this tag"
    )
    last_used_at: Optional[datetime] = Field(
        default=None,
        description="Last time this tag was applied to an item"
    )

    # === FLAGS ===
    is_system: bool = Field(
        default=False,
        description="True if this is a system-defined tag that shouldn't be deleted"
    )
    is_active: bool = Field(
        default=True,
        description="True if tag is active and available for use"
    )

    # === RELATIONSHIPS ===
    parts: List["PartModel"] = Relationship(
        back_populates="tags",
        link_model=PartTagLink,
    ...
    # === CONSTRAINTS ===
    __table_args__ = (
        UniqueConstraint('name_lower', name='uix_tag_name_lower'),
    )
