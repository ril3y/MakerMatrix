...
class LocationModel(SQLModel, table=True):
    ...
    container_capacity: Optional[int] = Field(
        default=None, description="Maximum quantity this container can hold (for capacity tracking)"
    )
    ...
    grid_rows: Optional[int] = Field(
        default=None, description="Number of rows in grid layout (e.g., 4 rows × 8 columns = 32 slots)"
    )
    grid_columns: Optional[int] = Field(default=None, description="Number of columns in grid layout")
    ...
    is_auto_generated_slot: bool = Field(
        default=False, description="True if this location was auto-created as a container slot"
    )
    ...
    is_smart_location: bool = Field(
        default=False, description="True for IoT-enabled locations with sensors and connectivity"
    )
    smart_device_id: Optional[str] = Field(
        default=None, description="ESP32 or other IoT device ID (e.g., 'esp32-cabinet-01')"
    )
    smart_slot_number: Optional[int] = Field(
        default=None, description="Slot number in smart cabinet (for multi-slot devices)"
    )
    ...
    auto_created: bool = Field(
        default=False, description="True if location was auto-created by smart device registration"
    )
    hidden_by_default: bool = Field(
        default=False, description="True to hide from main location list (for auto-created smart slots)"
    )

    # Connection tracking (for smart locations)
    is_connected: bool = Field(
        default=False, description="True if smart device is currently connected via WebSocket/MQTT"
    )
    last_seen: Optional[datetime] = Field(default=None, description="Last connection timestamp for smart device")
    firmware_version: Optional[str] = Field(default=None, description="Smart device firmware version")
    ...
    parent: Optional["LocationModel"] = Relationship(
        back_populates="children", sa_relationship_kwargs={"remote_side": "LocationModel.id"}
    )
    ...
    allocations: List["PartLocationAllocation"] = Relationship(
        back_populates="location", sa_relationship_kwargs={"cascade": "all, delete-orphan"}
    )
