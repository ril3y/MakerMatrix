...
class PartEnrichmentMetadata(SQLModel, table=True):
    ...
    part_id: str = Field(sa_column=Column(String, ForeignKey("partmodel.id", ondelete="CASCADE"), unique=True))
    ...
    capabilities_used: Optional[Dict[str, Any]] = Field(default_factory=dict, sa_column=Column(JSON))
...
class PartPricingHistory(SQLModel, table=True):
    ...
    pricing_tiers: Optional[Dict[str, Any]] = Field(
        default=None, sa_column=Column(JSON), description="Quantity-based pricing: {qty: price, ...}"
    )
