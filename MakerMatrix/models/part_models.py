...
class PartModel(SQLModel, table=True):
    ...
    component_type: Optional[str] = Field(
        default=None, index=True, description="Part type: resistor, capacitor, screwdriver, etc."
    )
    ...
    product_url: Optional[str] = (
        None  # URL to specific product page (e.g., https://boltdepot.com/Product-Details?product=15294)
    )
    ...
    emoji: Optional[str] = Field(
        default=None,
        max_length=50,
        description="Unicode emoji character or shortcode to use as visual icon (e.g., '🔩' or ':screw:')",
    )
    ...
    categories: List["CategoryModel"] = Relationship(
        back_populates="parts", link_model=PartCategoryLink, sa_relationship_kwargs={"lazy": "selectin"}
    )
    ...
    tags: List["TagModel"] = Relationship(
        back_populates="parts", sa_relationship_kwargs={"lazy": "selectin", "secondary": "part_tag_links"}
    )

    # Datasheet files (one-to-many relationship)
    datasheets: List["DatasheetModel"] = Relationship(
        back_populates="part", sa_relationship_kwargs={"lazy": "selectin", "cascade": "all, delete-orphan"}
    )

    # Projects (many-to-many relationship through link table)
    projects: List["ProjectModel"] = Relationship(
        back_populates="parts", link_model=PartProjectLink, sa_relationship_kwargs={"lazy": "selectin"}
    )
    ...
    pricing_history: List["PartPricingHistory"] = Relationship(
        back_populates="part", sa_relationship_kwargs={"lazy": "noload", "cascade": "all, delete-orphan"}
    )
    ...
    order_items: List["OrderItemModel"] = Relationship(back_populates="part", sa_relationship_kwargs={"lazy": "noload"})

    # Order summary information (one-to-one relationship)
    order_summary: Optional["PartOrderSummary"] = Relationship(
        back_populates="part", sa_relationship_kwargs={"lazy": "noload", "uselist": False}
    )
    ...
    def from_json(cls, data: Dict[str, Any]) -> "PartModel":
        ...
        if "categories" in data_copy and isinstance(data_copy["categories"], list):
            ...
            data_copy["categories"] = [
                CategoryModel(**item) if isinstance(item, dict) else item for item in data_copy["categories"]
            ]
    ...
    def to_dict(self, include: List[str] = None) -> Dict[str, Any]:
        ...
        exclude_fields = {
            ...
            "order_items",
            "order_summary",
            "allocations",  # Allocations included separately if needed
        }
            ...
            base_dict["allocations"] = [
                {
                    ...
                    "location_name": (
                        getattr(alloc.location, "name", None) if hasattr(alloc, "location") and alloc.location else None
                    ),
                ...
                if current_pricing:
                    base_dict["current_pricing"] = {
                        supplier_pricing.supplier: {
                            ...
                            "last_updated": (
                                supplier_pricing.created_at.isoformat() if supplier_pricing.created_at else None
                            ),
                        }
                ...
                base_dict["order_history"] = [
                    {
                        ...
                        "order_date": (
                            item.order.order_date.isoformat() if item.order and item.order.order_date else None
                        ),
    ...
    def update_data_quality_score(self):
        ...
        spec_score = (
            min(len(additional_props) / 5, 1.0) if additional_props else 0.0
        )  # Up to 5 properties for full score
    ...
    def get_enrichment_status(self) -> Dict[str, Any]:
        ...
        return {
            "last_enrichment": (
                enrichment_meta.last_enrichment_date.isoformat()
                if enrichment_meta and enrichment_meta.last_enrichment_date
                else None
            ),
