...
class OrderModel(SQLModel, table=True):
    ...
    order_metadata: Optional[Dict[str, Any]] = Field(default_factory=dict, sa_column=Column(JSON))

    # Relationships
    order_items: List["OrderItemModel"] = Relationship(
        back_populates="order", sa_relationship_kwargs={"cascade": "all, delete-orphan", "lazy": "selectin"}
    )
...
class OrderItemModel(SQLModel, table=True):
    ...
    part_id: Optional[str] = Field(
        default=None, sa_column=Column(String, ForeignKey("partmodel.id", ondelete="SET NULL"))
    )
    ...
    # Additional metadata
    properties: Optional[Dict[str, Any]] = Field(default_factory=dict, sa_column=Column(JSON))

    # Relationships
    order: Optional[OrderModel] = Relationship(back_populates="order_items")
    part: Optional["PartModel"] = Relationship(sa_relationship_kwargs={"lazy": "selectin"})
