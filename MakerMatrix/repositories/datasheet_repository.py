...
class DatasheetRepository(BaseRepository[DatasheetModel]):
    ...
    def get_datasheet_by_part_and_url(self, session: Session, part_id: str, 
                                     source_url: str) -> Optional[DatasheetModel]:
        """Get datasheet by part ID and source URL."""
        query = select(DatasheetModel).where(
            and_(
                DatasheetModel.part_id == part_id,
                DatasheetModel.source_url == source_url
            )
        )
    ...
    def get_failed_datasheets(self, session: Session, part_id: str = None) -> List[DatasheetModel]:
        ...
        query = select(DatasheetModel).where(
            and_(
                DatasheetModel.is_downloaded == False,
                DatasheetModel.download_error.is_not(None)
            )
        )
    ...
    def mark_download_failed(self, session: Session, datasheet_id: str, 
                           error_message: str) -> Optional[DatasheetModel]:
    ...
    def mark_download_successful(self, session: Session, datasheet_id: str, 
                                file_uuid: str, file_size: int, 
                                original_filename: str, file_extension: str) -> Optional[DatasheetModel]:
