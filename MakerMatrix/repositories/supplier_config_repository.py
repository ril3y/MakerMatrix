...
class SupplierConfigRepository(BaseRepository[SupplierConfigModel]):
        ...
        try:
            ...
            existing = session.exec(
                select(SupplierConfigModel).where(
                    SupplierConfigModel.supplier_name.ilike(config.supplier_name)
                )
            ).first()
            ...
            raise SupplierConfigAlreadyExistsError(
                f"Supplier configuration '{config.supplier_name}' already exists"
            )
    ...
    def update_test_status(self, session: Session, supplier_name: str, 
                          test_status: str, test_time: datetime) -> bool:
        """
    ...
    def get_enabled_suppliers(self, session: Session) -> List[str]:
        ...
        configs = session.exec(
            select(SupplierConfigModel).where(SupplierConfigModel.enabled == True)
        ).all()
    ...
    def bulk_update_enabled_status(self, session: Session, supplier_names: List[str], 
                                  enabled: bool) -> int:
        """
        ...
        configs = session.exec(
            select(SupplierConfigModel).where(
                SupplierConfigModel.supplier_name.in_(supplier_names)
            )
        ).all()
...
class EnrichmentProfileRepository(BaseRepository[EnrichmentProfileModel]):
    ...
    def get_by_name(self, session: Session, name: str, user_id: str = None) -> Optional[EnrichmentProfileModel]:
        ...
        if user_id:
            query = query.where(
                and_(
                    EnrichmentProfileModel.created_by_user_id == user_id,
                    EnrichmentProfileModel.is_public == True
                )
            )
    ...
    def get_user_profiles(self, session: Session, user_id: str) -> List[EnrichmentProfileModel]:
        ...
        profiles = session.exec(
            select(EnrichmentProfileModel).where(
                EnrichmentProfileModel.created_by_user_id == user_id
