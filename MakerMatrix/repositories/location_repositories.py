...
class LocationRepository:
    ...
    def get_location_hierarchy(session: Session, location_id: str) -> Dict[str, Any]:
        ...
        location = session.exec(
            select(LocationModel).options(selectinload(LocationModel.children)).where(LocationModel.id == location_id)
        ).first()
        ...
        return {"hierarchy": hierarchy, "affected_location_ids": affected_ids}
    ...
    def get_affected_part_ids(session: Session, location_ids: List[str]) -> List[str]:
        ...
        parts = session.exec(select(PartModel.id).where(PartModel.location_id.in_(location_ids))).all()
    ...
    def get_location(session: Session, location_query: LocationQueryModel) -> Optional[LocationModel]:
        if location_query.id:
            ...
            raise InvalidReferenceError(
                status="error", message="Either 'id' or 'name' must be provided for location lookup", data=None
            )
        if location:
            return location
        else:
            ...
            raise ResourceNotFoundError(status="error", message=f"Location {location_id_or_name} not found", data=None)
    ...
    def add_location(session: Session, location_data: Dict[str, Any]) -> LocationModel:
        ...
        if new_location.parent_id:
            ...
            statement = (
                select(LocationModel)
                .options(selectinload(LocationModel.parent))
                .where(LocationModel.id == new_location.id)
            )
    ...
    def get_location_details(session: Session, location_id: str) -> Dict[str, Any]:
        ...
        """
        location = session.exec(
            select(LocationModel).options(joinedload(LocationModel.children)).where(LocationModel.id == location_id)
        ).first()

        if not location:
            raise ResourceNotFoundError(
                status="error", message=f"Location with ID '{location_id}' not found", data=None
            )
    ...
    def update_location(session: Session, location_id: str, location_data: Dict[str, Any]) -> LocationModel:
        ...
        location = session.get(LocationModel, location_id)
        if not location:
            raise ResourceNotFoundError(status="error", message="Location not found", data=None)
    ...
    def cleanup_locations(session: Session) -> int:
        ...
        invalid_locations = [loc for loc in all_locations if loc.parent_id and loc.parent_id not in valid_ids]
