...
class ActivityRepository(BaseRepository[ActivityLogModel]):
    ...
    def cleanup_old_activities(self, session: Session, keep_days: int = 90) -> int:
        ...
        statement = select(ActivityLogModel).where(ActivityLogModel.timestamp < cutoff_date)
    ...
    def get_activity_statistics(self, session: Session, hours: int = 24) -> Dict[str, Any]:
        """
