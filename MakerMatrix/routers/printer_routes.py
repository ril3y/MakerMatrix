    ...
    drivers_data = [
        ...
        },
        {
            ...
            "custom_fields": {
                ...
                "heat_setting": {"type": "range", "min": 1, "max": 10, "default": 5, "label": "Heat Setting"},
                "parity": {"type": "select", "options": ["none", "even", "odd"], "default": "none", "label": "Parity"},
                "stop_bits": {"type": "select", "options": [1, 2], "default": 1, "label": "Stop Bits"},
            },
        },
    ]
...
async def register_printer(registration: PrinterRegistration, request: Request = None):
    ...
    if existing_printer:
        raise HTTPException(status_code=409, detail=f"Printer with ID '{registration.printer_id}' already exists")
    ...
    await activity_service.log_printer_registered(
        printer_id=registration.printer_id, printer_name=registration.name, user=current_user, request=request
    )
...
async def print_text_label(
    request: TextLabelRequest, http_request: Request = None, current_user: UserModel = Depends(get_current_user)
):
    """Print a text label."""
    print(f"Print request: {request}")
    result = await printer_manager.print_text_label(
        printer_id=request.printer_id, text=request.text, label_size=request.label_size, copies=request.copies
    )
...
async def print_qr_code(request: QRCodeRequest):
    ...
    result = await printer_manager.print_qr_code(
        printer_id=request.printer_id, data=request.data, label_size=request.label_size, copies=request.copies
    )
...
async def print_image_label(
    ...
    result = await printer_manager.print_image(
        printer_id=printer_id, image_data=image_data, label_size=label_size, copies=copies
    )
...
async def print_template_label(
    request: TemplatePrintRequest, http_request: Request = None, current_user: UserModel = Depends(get_current_user)
):
...
async def preview_template_label(request: TemplatePreviewRequest):
    ...
    result = await printer_manager.preview_template_label(template_id=request.template_id, data=request.data)
...
async def print_advanced_label(
    request: AdvancedLabelRequest, http_request: Request = None, current_user: UserModel = Depends(get_current_user)
):
