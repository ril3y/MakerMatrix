...
@router.get("/parts/{part_id}/allocations", response_model=ResponseSchema, summary="Get all allocations for a part")
async def get_part_allocations(
    ...
    return ResponseSchema(status="success" if result.success else "error", message=result.message, data=result.data)


@router.post("/parts/{part_id}/allocations", response_model=ResponseSchema, summary="Create new allocation")
...
@router.put("/parts/{part_id}/allocations/{allocation_id}", response_model=ResponseSchema, summary="Update allocation")
...
@router.delete(
    "/parts/{part_id}/allocations/{allocation_id}", response_model=ResponseSchema, summary="Delete allocation"
)
...
@router.post("/parts/{part_id}/transfer", response_model=ResponseSchema, summary="Transfer quantity between locations")
async def transfer_quantity(
    ...
    transfer_data = TransferRequest(
        from_location_id=from_location_id, to_location_id=to_location_id, quantity=quantity, notes=notes
    )
