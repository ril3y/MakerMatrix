...
@router.post("/suppliers/{supplier_name}/test", response_model=ResponseSchema[ConnectionTestResult])
async def test_supplier_connection(
    ...
    try:
        service = SupplierConfigService()
        result = await service.test_supplier_connection(supplier_name)
        
        return ResponseSchema(
            status="success" if result["success"] else "warning",
            message=f"Connection test {'successful' if result['success'] else 'failed'} for {supplier_name}",
            data=result
        )
        ...
        logger.error(f"Error testing supplier connection {supplier_name}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to test supplier connection"
        )


@router.get("/suppliers/{supplier_name}/capabilities", response_model=ResponseSchema[Dict[str, Any]])
async def get_supplier_actual_capabilities(
    supplier_name: str,
    current_user: UserModel = Depends(get_current_user)
):
    """Get the actual capabilities supported by the supplier's API client"""
    try:
        service = SupplierConfigService()
        ...
        if not supplier_config.enabled:
            return ResponseSchema(
                status="warning",
                message=f"Supplier {supplier_name} is disabled",
                data={
        ...
        try:
            ...
            client = service._create_api_client(supplier_config, credentials)
            
            # Get actual capabilities from the client
            actual_capabilities = client.get_supported_capabilities()
            ...
            return ResponseSchema(
                ...
                data={
                    ...
                    "capabilities_match": set(configured_capabilities) == set(actual_capabilities),
                    "missing_in_config": list(set(actual_capabilities) - set(configured_capabilities)),
                    "extra_in_config": list(set(configured_capabilities) - set(actual_capabilities))
                }
            )
        except Exception as e:
            return ResponseSchema(
                status="error",
                message=f"Failed to get capabilities for {supplier_name}",
                data={
                    "supplier_name": supplier_name,
                    "enabled": True,
                    "configured_capabilities": supplier_config.get_capabilities(),
