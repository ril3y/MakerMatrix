...
async def get_tasks(
    ...
    return {"status": "success", "data": tasks, "total": len(tasks), "limit": limit, "offset": offset}
...
async def get_my_tasks(
    ...
    try:
        ...
        return {"status": "success", "data": tasks, "total": len(tasks)}
...
async def get_available_task_types(current_user: UserModel = Depends(require_permission("tasks:read"))):
    ...
    try:
        ...
        return {"status": "success", "data": task_types}
    except Exception as e:
...
async def get_task_stats(current_user: UserModel = Depends(require_permission("tasks:read"))):
    ...
    try:
        ...
        for task in tasks:
            ...
            # Completed today
            if task["status"] == TaskStatus.COMPLETED and task.get("completed_at"):
...
async def start_task_worker(
    background_tasks: BackgroundTasks, current_user: UserModel = Depends(require_permission("tasks:admin"))
):
    """Start the task worker (admin only)"""
    try:
        if task_service.is_worker_running:
            return {"status": "info", "message": "Task worker is already running"}
        ...
        return {"status": "success", "message": "Task worker started successfully"}
    except Exception as e:
...
async def stop_task_worker(current_user: UserModel = Depends(require_permission("tasks:admin"))):
    """Stop the task worker (admin only)"""
    try:
        if not task_service.is_worker_running:
            return {"status": "info", "message": "Task worker is not running"}

        await task_service.stop_worker()

        logger.info(f"Task worker stopped by user {current_user.username}")

        return {"status": "success", "message": "Task worker stopped successfully"}
    except Exception as e:
...
async def get_worker_status(current_user: UserModel = Depends(require_permission("tasks:read"))):
...
async def quick_create_part_enrichment_task(
    request: Dict[str, Any], current_user: UserModel = Depends(require_permission("tasks:create"))
):
...
async def quick_create_database_backup_task(
    request: Dict[str, Any], current_user: UserModel = Depends(require_permission("admin"))  # Requires admin permission
):
...
async def get_supplier_capabilities(current_user: UserModel = Depends(require_permission("tasks:read"))):
...
async def get_supplier_specific_capabilities(
    supplier_name: str, current_user: UserModel = Depends(require_permission("tasks:read"))
):
    ...
    try:
        ...
        return {"status": "success", "message": f"Retrieved capabilities for {supplier_name}", "data": capabilities}
    except Exception as e:
        logger.error(f"Failed to get capabilities for {supplier_name}: {e}", exc_info=True)
        raise HTTPException(
            status_code=404, detail=f"Supplier '{supplier_name}' not found or failed to get capabilities"
        )


@router.get("/capabilities/find/{capability_type}", response_model=Dict[str, Any])
async def find_suppliers_with_capability(
    capability_type: str, current_user: UserModel = Depends(require_permission("tasks:read"))
):
...
async def get_user_task_permissions(current_user: UserModel = Depends(require_permission("tasks:create"))):
...
async def get_user_task_limits(current_user: UserModel = Depends(require_permission("tasks:create"))):
...
async def get_task(task_id: str, current_user: UserModel = Depends(require_permission("tasks:read"))):
    ...
    try:
        ...
        return {"status": "success", "data": task_response.data}
...
async def update_task(
    ...
    try:
        ...
        return {"status": "success", "message": "Task updated successfully", "data": task.to_dict()}
    except HTTPException:
        raise
    except Exception as e:
...
async def cancel_task(task_id: str, current_user: UserModel = Depends(require_permission("tasks:update"))):
...
async def retry_task(task_id: str, current_user: UserModel = Depends(require_permission("tasks:update"))):
    """Retry a failed task"""
    try:
        ...
        return {"status": "success", "message": "Task retry scheduled successfully"}
    except HTTPException:
        raise
    except Exception as e:
...
async def delete_task(task_id: str, current_user: UserModel = Depends(require_permission("tasks:delete"))):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail="Task cannot be deleted (task is still running or does not exist)"
            )

        logger.info(f"Deleted task {task_id} by user {current_user.username}")

        return ResponseSchema(status="success", message="Task deleted successfully", data={"task_id": task_id})
    except HTTPException:
        raise
    except Exception as e:
