...
# Enrichment capabilities endpoints
...
async def get_supplier_capability(
    supplier_name: str,
    current_user: UserModel = Depends(require_permission("tasks:read"))
):
    ...
    try:
        ...
        logger.error(f"Failed to get capabilities for supplier {supplier_name}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to get supplier capabilities: {str(e)}")
