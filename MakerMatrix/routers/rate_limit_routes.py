...
async def get_all_supplier_usage(current_user: UserModel = Depends(get_current_user)):
    ...
    return base_router.build_success_response(
        message=f"Retrieved usage statistics for {len(usage_data)} suppliers", data=usage_data
    )
...
async def get_supplier_usage(
    supplier_name: str, time_period: str = "24h", current_user: UserModel = Depends(get_current_user)
):
    ...
    return base_router.build_success_response(
        message=f"Retrieved usage statistics for {supplier_name}", data=response_data
    )


@router.get("/suppliers/{supplier_name}/status", response_model=ResponseSchema[Dict[str, Any]])
@standard_error_handling
async def get_supplier_rate_limit_status(supplier_name: str, current_user: UserModel = Depends(get_current_user)):
    """Get current rate limit status for a supplier (for real-time monitoring)"""
    rate_status = await rate_limit_service.check_rate_limit(supplier_name.upper())

    return base_router.build_success_response(message=f"Rate limit status for {supplier_name}", data=rate_status)


@router.post("/initialize", response_model=ResponseSchema[Dict[str, Any]])
@standard_error_handling
async def initialize_default_rate_limits(current_user: UserModel = Depends(get_current_user)):
    """Initialize default rate limits for known suppliers"""
    await rate_limit_service.initialize_default_limits()

    return base_router.build_success_response(
        message="Default rate limits initialized successfully", data={"initialized": True}
    )


@router.get("/summary", response_model=ResponseSchema[Dict[str, Any]])
@standard_error_handling
async def get_rate_limit_summary(current_user: UserModel = Depends(get_current_user)):
        ...
        for period, usage in supplier_data["usage_percentage"].items():
            if usage > 80:  # More than 80% of limit used
                approaching_limits.append(
                    {"supplier": supplier_data["supplier_name"], "period": period, "usage_percentage": usage}
                )
    ...
    return base_router.build_success_response(message="Rate limiting summary retrieved successfully", data=summary)
