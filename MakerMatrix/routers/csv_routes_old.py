...
from MakerMatrix.dependencies.auth import require_permission, get_current_user
...
@router.post("/preview", response_model=ResponseSchema[ImportPreviewResult])
async def preview_csv_content(
    ...
    try:
        ...
        # Parse CSV using parse_csv_to_parts method
        parsed_data, errors = csv_import_service.parse_csv_to_parts(
            csv_content, 
            parser_type,
            enable_enrichment=False  # No enrichment for preview
        )
...
@router.post("/import", response_model=ResponseSchema[ImportResult])
async def import_csv_data(
    ...
    try:
        ...
        # Parse CSV using parse_csv_to_parts method
        parsed_data, parsing_errors = csv_import_service.parse_csv_to_parts(
            request.csv_content, 
            parser_type,
            enable_enrichment=False  # No enrichment during import
        )
        
        if parsing_errors:
            logger.warning(f"CSV parsing errors: {parsing_errors}")
        
        # If no data parsed, return early
        if not parsed_data:
            result = ImportResult(
                import_id=import_id,
                status="failed",
                ...
                failed_count=1,
                part_ids=[],
                failed_items=[{"error": "No valid parts data found in CSV"}],
            ...
            return ResponseSchema(
                status="error",
                message="No valid parts found in CSV",
                data=result
            )
        
        # Import using the service method that handles order creation
        part_service = PartService()
        success_parts, failed_parts, imported_part_ids = await csv_import_service.import_parts_with_order(
            parsed_data,
            part_service,
            request.order_info or {}
        )
        
        # Extract order_id from the first success message if created
        order_id = None
        # The order_id would be in the order_info if it was created
        
        # Convert success_parts strings to expected format
        success_items = []
        for i, part_id in enumerate(imported_part_ids):
            if i < len(parsed_data):
                success_items.append({
                    "part_number": parsed_data[i].get("part_number", "Unknown"),
                    "part_id": part_id
                })
        
        # Convert failed_parts strings to expected format
        failed_items = []
        for failed_msg in failed_parts:
            failed_items.append({
                "error": failed_msg,
                "part_number": "Unknown"  # Extract from message if needed
            })
...
async def parse_csv_only(
    ...
    try:
        ...
        from MakerMatrix.services.parser_client_registry import supports_enrichment
...
@router.post("/import-file", response_model=ResponseSchema[ImportResult])
...
# Import Status endpoints (previously in import_routes.py)
# In-memory storage for import status (should use database in production)
_import_status = {}

@router.get("/import-status/{import_id}", response_model=ResponseSchema[Dict[str, Any]])
...
# Parser Information endpoints (previously in import_routes.py)
@router.get("/available-parsers", response_model=ResponseSchema[List[Dict[str, Any]]])
