...
async def websocket_admin_endpoint(websocket: WebSocket, token: Optional[str] = Query(None)):
...
async def handle_task_websocket_message(websocket: WebSocket, message: dict, user: UserModel = None):
    ...
    if message_type == "ping":
        await websocket_manager.send_to_connection(websocket, {"type": "pong", "timestamp": message.get("timestamp")})
        ...
        if task_id:
            ...
            await websocket_manager.send_to_connection(
                websocket, {"type": "task_subscription", "task_id": task_id, "status": "subscribed"}
            )

    elif message_type == "unsubscribe_task":
        task_id = message.get("task_id")
        await websocket_manager.send_to_connection(
            websocket, {"type": "task_unsubscription", "task_id": task_id, "status": "unsubscribed"}
        )

    elif message_type == "get_connection_info":
        stats = websocket_manager.get_connection_stats()
        await websocket_manager.send_to_connection(websocket, {"type": "connection_info", "data": stats})
...
async def handle_general_websocket_message(websocket: WebSocket, message: dict, user: UserModel = None):
    ...
    if message_type == "ping":
        ...
        await websocket_manager.send_to_connection(websocket, {"type": "activity_subscription", "status": "subscribed"})
...
async def handle_admin_websocket_message(websocket: WebSocket, message: dict, user: UserModel):
    ...
    if message_type == "get_system_stats":
        ...
        await websocket_manager.send_to_connection(websocket, {"type": "system_stats", "data": stats})

    elif message_type == "broadcast_notification":
        # Allow admin to broadcast notifications
        notification = message.get("notification", {})
        if notification:
            await websocket_manager.send_system_notification(notification, connection_types=["general", "tasks"])
