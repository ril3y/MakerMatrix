...
class TestQREnrichmentFix:
        ...
        try:
            with Session(engine) as session:
                for part_number in test_parts:
                    part = session.exec(select(PartModel).where(PartModel.part_number == part_number)).first()
                    if part:
                        session.delete(part)
                        session.commit()
    ...
    async def test_lcsc_qr_enrichment_with_specifications(self, enrichment_handlers, cleanup_test_parts):
        ...
        try:
            ...
            if isinstance(result, dict) and "error" in result:
                assert "Capabilities not supported" not in str(
                    result["error"]
    ...
    async def test_mouser_qr_enrichment_capabilities(self, enrichment_handlers, cleanup_test_parts):
        ...
        with Session(engine) as session:
            # Clean up any existing part
            existing_part = session.exec(select(PartModel).where(PartModel.part_number == "STM32F373CBT6")).first()
            if existing_part:
                session.delete(existing_part)
                session.commit()
        ...
        task_data = {
            "part_id": part_id,
            "supplier": "mouser",
            "capabilities": ["fetch_datasheet", "fetch_image", "fetch_pricing", "fetch_stock", "fetch_specifications"],
        }
    ...
    def test_capability_name_consistency_fixed(self):
        ...
        for capability, task_type in critical_mappings.items():
            assert (
                capability.value == task_type.value
    ...
    async def test_enrichment_with_invalid_capability(self, enrichment_handlers, cleanup_test_parts):
        ...
        task_data = {"part_id": part_id, "supplier": "lcsc", "capabilities": ["invalid_capability", "fetch_datasheet"]}
        ...
        try:
            ...
            if isinstance(result, dict) and "error" in result:
                assert "invalid_capability" in str(result["error"]) or "Capabilities not supported" in str(
                    result["error"]
                )
