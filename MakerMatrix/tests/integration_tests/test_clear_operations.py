...
def non_admin_headers(test_client):
    ...
    user_data = {"username": "testuser", "email": "test@example.com", "password": "TestPass123!", "roles": ["user"]}
...
def setup_test_data():
    ...
    with Session(engine) as session:
        ...
        test_category = CategoryModel(name="Test_Clear_Category", description="Test category for clear operations")
        session.add(test_category)

        # Create test supplier config
        test_supplier_config = SupplierConfigModel(
            supplier_name="test_supplier", config_data={"api_key": "test_key", "base_url": "test.com"}
        )
        session.add(test_supplier_config)

        # Create test supplier credentials
        test_credentials = SupplierCredentialsModel(
            supplier_name="test_supplier", credentials_data_encrypted=b"encrypted_test_data"
        )
