...
SAMPLE_CATEGORY_DATA = {"name": "Test Category", "description": "Test category description"}
...
SAMPLE_USER_DATA = {"username": "newuser", "email": "newuser@example.com", "password": "NewPass123!", "roles": ["user"]}
...
def user_token():
    ...
    login_response = client.post(
        "/auth/login", json={"username": unique_user["username"], "password": unique_user["password"]}
    )
...
class TestAuthenticationRoutes:
    ...
    def test_login_form_data_success(self):
        ...
        response = client.post(
            "/auth/login", data={"username": ADMIN_USER["username"], "password": ADMIN_USER["password"]}
        )
        assert response.status_code == 200
        data = response.json()
        assert "access_token" in data
    ...
    def test_token_expiration_simulation(self):
        ...
        expired_token = auth_service.create_access_token(data={"sub": "admin"}, expires_delta=timedelta(minutes=-1))
...
class TestUserManagementRoutes:
    ...
    def test_update_user(self, setup_test_data, admin_token):
        ...
        headers = get_auth_headers(admin_token)
        update_data = {"email": f"updated_{str(uuid.uuid4())[:8]}@example.com", "is_active": True, "roles": ["user"]}
    ...
    def test_update_user_duplicate_email(self, setup_test_data, admin_token):
        ...
        update_data = {"email": setup_test_data["admin_user"].email, "is_active": True, "roles": ["user"]}
        response = client.put(f"/api/users/{user_id}", json=update_data, headers=headers)
    ...
    def test_update_password(self, setup_test_data, admin_token):
        ...
        user_id = setup_test_data["admin_user"].id
        headers = get_auth_headers(admin_token)
        password_data = {"current_password": ADMIN_USER["password"], "new_password": "NewAdmin123!"}
        ...
        login_response = client.post("/auth/login", json={"username": "admin", "password": "NewAdmin123!"})
        assert login_response.status_code == 200

        # Change password back to original for other tests
        reset_data = {"current_password": "NewAdmin123!", "new_password": ADMIN_USER["password"]}
    ...
    def test_update_password_wrong_current(self, setup_test_data, admin_token):
        ...
        user_id = setup_test_data["admin_user"].id
        headers = get_auth_headers(admin_token)
        password_data = {"current_password": "WrongPassword123!", "new_password": "NewAdmin123!"}
        response = client.put(f"/api/users/{user_id}/password", json=password_data, headers=headers)
    ...
    def test_update_password_unauthorized(self, user_token):
        ...
        fake_id = str(uuid.uuid4())
        headers = get_auth_headers(user_token)
        password_data = {"current_password": "current", "new_password": "new"}
...
class TestPartsManagementRoutes:
    ...
    def test_update_part(self, admin_token):
        ...
        part_data = {
            **SAMPLE_PART_DATA,
            "part_name": f"Test Update Part {unique_suffix}",
            "part_number": f"UPD001{unique_suffix}",
        }
        create_response = client.post("/api/parts/add_part", json=part_data, headers=headers)
        assert create_response.status_code == 200
        ...
        # Now update the part
        update_data = {"description": "Updated description", "quantity": 150}
    ...
    def test_search_parts(self, admin_token):
        ...
        headers = get_auth_headers(admin_token)
        search_data = {"search_term": "resistor", "page": 1, "page_size": 10}
    ...
    def test_delete_part(self, admin_token):
        ...
        part_data = {
            **SAMPLE_PART_DATA,
            "part_name": f"Test Delete Part {unique_suffix}",
            "part_number": f"DEL001{unique_suffix}",
        }
        create_response = client.post("/api/parts/add_part", json=part_data, headers=headers)
        assert create_response.status_code == 200
        part_id = create_response.json()["data"]["id"]
...
class TestCategoriesManagementRoutes:
    ...
    def test_update_category(self, admin_token):
        ...
        update_data = {"name": f"Updated Category {unique_suffix}", "description": "Updated description"}
    ...
    def test_add_category_missing_name(self, admin_token):
        ...
        headers = get_auth_headers(admin_token)
        category_data = {"description": "Test category without name"}
        response = client.post("/api/categories/add_category", json=category_data, headers=headers)
...
class TestLocationsManagementRoutes:
    ...
    def test_update_location(self, admin_token):
        ...
        update_data = {"name": f"Updated Location {unique_suffix}", "description": "Updated description"}
...
class TestTaskManagementRoutes:
    ...
    def test_quick_part_enrichment_task(self, setup_test_data, admin_token):
        ...
        headers = get_auth_headers(admin_token)
        task_data = {"part_id": setup_test_data["part"].id, "supplier": "LCSC", "capabilities": ["fetch_datasheet"]}
    ...
    def test_quick_datasheet_fetch_task(self, setup_test_data, admin_token):
        ...
        headers = get_auth_headers(admin_token)
        task_data = {"part_id": setup_test_data["part"].id, "supplier": "LCSC"}
    ...
    def test_quick_database_backup_task(self, setup_test_data, admin_token):
        ...
        headers = get_auth_headers(admin_token)
        task_data = {"backup_name": "test_backup", "include_datasheets": True, "include_images": True}
...
class TestImportRoutes:
    ...
    def test_csv_import_without_content(self, setup_test_data, admin_token):
        ...
        # This test is expected to fail because the legacy CSV API has been deprecated
        headers = get_auth_headers(admin_token)
        import_data = {"csv_content": "", "parser_type": "lcsc"}
    ...
    def test_update_csv_config(self, setup_test_data, admin_token):
        ...
        # This test is expected to fail because the legacy CSV API has been deprecated
        headers = get_auth_headers(admin_token)
        config_data = {"download_datasheets": True, "download_images": False, "overwrite_existing_files": False}
...
class TestAIIntegrationRoutes:
    ...
    def test_ai_chat_disabled(self, setup_test_data, admin_token):
        ...
        headers = get_auth_headers(admin_token)
        chat_data = {"message": "Hello, AI!", "conversation_history": []}
...
class TestPrinterRoutes:
    ...
    def test_print_text(self, mock_printer_manager, admin_token):
        ...
        headers = get_auth_headers(admin_token)
        print_data = {"printer_id": "test-printer-123", "text": "Test Label Text", "label_size": "29x90", "copies": 1}
    ...
    def test_test_setup(self, mock_printer_manager, admin_token):
        ...
        mock_printer_manager.test_printer_setup.return_value = {
            "status": "success",
...
class TestSupplierManagementRoutes:
    ...
    def test_test_supplier_connection_without_config(self, admin_token):
        ...
        response = client.post("/api/suppliers/lcsc/test", json={"test_data": "minimal"}, headers=headers)
    ...
    def test_supplier_config_crud_operations(self, admin_token):
        ...
        response = client.post("/api/suppliers/config/suppliers", json=config_data, headers=headers)
    ...
    def test_supplier_part_operations_unauthorized(self, admin_token):
        ...
        response = client.post("/api/suppliers/lcsc/part/TEST-001", json={"config": {}}, headers=headers)
    ...
    def test_supplier_oauth_authorization_url(self, admin_token):
        ...
        response = client.post("/api/suppliers/digikey/oauth/authorization-url", json={"config": {}}, headers=headers)
    ...
    def test_supplier_error_handling(self, admin_token):
        ...
        response = client.post("/api/suppliers/config/suppliers", json={"invalid": "data"}, headers=headers)
        assert response.status_code == 422
        data = response.json()
        assert data["status"] == "error"
...
class TestAnalyticsRoutes:
    ...
    def test_spending_by_supplier(self, admin_token):
        ...
        response = client.get(
            "/api/analytics/spending/by-supplier?start_date=2024-01-01&end_date=2024-12-31&limit=10", headers=headers
        )
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "success"
    ...
    def test_spending_by_category(self, admin_token):
        ...
        response = client.get(
            "/api/analytics/spending/by-category?start_date=2024-01-01&end_date=2024-12-31", headers=headers
        )
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "success"
...
class TestActivityManagementRoutes:
    ...
    def test_activity_cleanup(self, admin_token):
        ...
        response = client.post("/api/activity/cleanup", json={"keep_days": 90}, headers=headers)
        # May return 500 if database session issues
        assert response.status_code in [200, 500]
        data = response.json()
        if response.status_code == 200:
        ...
        response = client.post("/api/activity/cleanup", json={}, headers=headers)
        assert response.status_code in [200, 500]
        data = response.json()
        if response.status_code == 200:
            assert data["status"] == "success"
...
class TestLabelPreviewRoutes:
    ...
    def test_part_qr_code_preview(self, mock_get_part, admin_token, sample_part):
        ...
        request_data = {"label_size": "29x90", "printer_id": "ql-800"}

        response = client.post(f"/api/preview/part/qr_code/{sample_part.id}", json=request_data, headers=headers)
    ...
    def test_part_name_preview(self, mock_get_part, admin_token, sample_part):
        ...
        response = client.post(f"/api/preview/part/name/{sample_part.id}", json=request_data, headers=headers)
        assert response.status_code in [200, 422]  # May fail if printer service not available
    ...
    def test_text_preview(self, admin_token):
        ...
        request_data = {"text": "Test Label", "label_size": "29x90", "printer_id": "ql-800"}

        response = client.post("/api/preview/text", json=request_data, headers=headers)
        assert response.status_code in [
            200,
            422,
            500,
        ]  # May fail if printer service not available or invalid label size
    ...
    def test_part_combined_preview(self, mock_get_part, admin_token, sample_part):
        ...
        request_data = {"custom_text": "Custom Label Text", "label_size": "29x90", "printer_id": "ql-800"}

        response = client.post(f"/api/preview/part/combined/{sample_part.id}", json=request_data, headers=headers)
        assert response.status_code in [200, 422]  # May fail if printer service not available
...
class TestFrontendRoutes:
    ...
    def test_spa_routes(self):
        ...
        frontend_routes = ["/dashboard", "/parts", "/categories", "/locations", "/inventory", "/settings"]
    ...
    def test_static_assets(self):
        ...
        static_files = ["/static/css/main.css", "/static/js/main.js", "/assets/favicon.ico"]
