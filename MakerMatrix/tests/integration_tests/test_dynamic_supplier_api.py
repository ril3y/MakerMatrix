...
class TestSupplierAPIIntegration:
    ...
    def test_get_individual_supplier_info(self, auth_headers):
        ...
        for supplier_name in ["digikey", "lcsc", "mouser"]:
            response = requests.get(
                f"{API_BASE_URL}/api/suppliers/{supplier_name}/info",
    ...
    def test_get_credential_schemas(self, auth_headers):
        ...
        response = requests.get(
            f"{API_BASE_URL}/api/suppliers/digikey/credentials-schema",
            headers=auth_headers
        )
        assert response.status_code == 200
        ...
        response = requests.get(
            f"{API_BASE_URL}/api/suppliers/lcsc/credentials-schema",
            headers=auth_headers
        )
        assert response.status_code == 200
        ...
        response = requests.get(
            f"{API_BASE_URL}/api/suppliers/mouser/credentials-schema",
            headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_get_configuration_schemas(self, auth_headers):
        ...
        for supplier_name in ["digikey", "lcsc", "mouser"]:
            response = requests.get(
                f"{API_BASE_URL}/api/suppliers/{supplier_name}/config-schema",
                headers=auth_headers
            )
            assert response.status_code == 200
    ...
    def test_get_supplier_capabilities(self, auth_headers):
        ...
        for supplier_name in ["digikey", "lcsc", "mouser"]:
            response = requests.get(
                f"{API_BASE_URL}/api/suppliers/{supplier_name}/capabilities",
                headers=auth_headers
            )
            assert response.status_code == 200
    ...
    def test_frontend_integration_scenario(self, auth_headers):
        ...
        for supplier_name, supplier_info in suppliers_info.items():
            ...
            cred_response = requests.get(
                f"{API_BASE_URL}/api/suppliers/{supplier_name}/credentials-schema",
                headers=auth_headers
            )
    ...
    def test_error_handling(self, auth_headers):
        ...
        response = requests.get(
            f"{API_BASE_URL}/api/suppliers/invalid_supplier/info",
            headers=auth_headers
        )
...
def test_api_endpoints_are_registered():
    ...
    try:
        ...
        for expected_path in expected_paths:
            ...
            path_exists = any(
                expected_path.replace("{supplier_name}", "{supplier_name}") in path or
                expected_path.replace("{supplier_name}", "{supplier_name:path}") in path
