...
from MakerMatrix.models.rate_limiting_models import SupplierRateLimitModel, SupplierUsageTrackingModel
...
class TestCompleteEnrichmentSystem:
    ...
    async def test_rate_limiting_system_initialization(self, engine):
        ...
        with Session(engine) as session:
            ...
            print(
                f"✅ MOUSER rate limits: {mouser_limits.requests_per_minute}/min, {mouser_limits.requests_per_hour}/hr"
            )
    ...
    async def test_websocket_integration(self, engine):
        ...
        task_id = await queue_manager.queue_part_enrichment(
            part_id="ws-test", part_name="WebSocket Test Part", supplier_name="MOUSER", capabilities=["fetch_datasheet"]
        )
    ...
    def test_mouser_api_protection_design(self):
        ...
        expected_mouser_limits = {"requests_per_minute": 30, "requests_per_hour": 1000, "requests_per_day": 1000}
        ...
        print(
            f"   • Rate Limits: {expected_mouser_limits['requests_per_minute']}/min, {expected_mouser_limits['requests_per_hour']}/hr"
        )
