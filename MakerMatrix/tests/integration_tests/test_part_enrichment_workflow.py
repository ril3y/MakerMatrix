...
class TestPartEnrichmentWorkflow:
    ...
    async def test_complete_enrichment_workflow(self, client, test_user, test_part, auth_headers):
        ...
        with (
            patch("MakerMatrix.auth.dependencies.auth.get_current_user", return_value=test_user),
            ...
            response = client.post("/tasks/quick/part-enrichment", json=enrichment_data, headers=auth_headers)
    ...
    async def test_bulk_enrichment_workflow(self, client, test_user, auth_headers):
        ...
        test_parts = [PartModel(id=f"part_{i}", name=f"Part {i}", part_number=f"P{i:03d}") for i in range(1, 4)]
        ...
        with (
            ...
            response = client.post("/tasks/quick/bulk-enrichment", json=bulk_data, headers=auth_headers)
