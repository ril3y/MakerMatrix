...
def regular_user():
    ...
    return UserModel(
        id="user_id", username="user", email="user@test.com", hashed_password="fake_hash", role=role, is_active=True
    )
...
def admin_user():
    ...
    return UserModel(
        id="admin_id", username="admin", email="admin@test.com", hashed_password="fake_hash", role=role, is_active=True
    )
...
class TestTaskSecurityEndpoints:
    ...
    def test_regular_user_can_create_part_enrichment(self, client, regular_user, auth_headers):
        ...
        with (
            patch("MakerMatrix.auth.dependencies.auth.get_current_user", return_value=regular_user),
            patch("MakerMatrix.routers.task_routes.require_permission") as mock_require_permission,
            patch(
                "MakerMatrix.services.task_security_service.task_security_service.validate_task_creation"
            ) as mock_validate,
            ...
            task_data = {"part_id": "test_part", "supplier": "LCSC", "capabilities": ["fetch_datasheet"]}

            response = client.post("/tasks/quick/part-enrichment", json=task_data, headers=auth_headers)
    ...
    def test_regular_user_cannot_create_admin_tasks(self, client, regular_user, auth_headers):
        ...
        with (
            ...
            mock_validate.return_value = (
                False,
                "Insufficient permissions. Missing: admin, database:cleanup, tasks:admin",
            )

            task_data = {"cleanup_options": {"remove_old_tasks": True}}

            response = client.post("/tasks/quick/database-cleanup", json=task_data, headers=auth_headers)
    ...
    def test_power_user_can_create_bulk_enrichment(self, client, power_user, auth_headers):
        ...
        with (
            patch("MakerMatrix.auth.dependencies.auth.get_current_user", return_value=power_user),
            patch("MakerMatrix.routers.task_routes.require_permission") as mock_require_permission,
            patch(
                "MakerMatrix.services.task_security_service.task_security_service.validate_task_creation"
            ) as mock_validate,
            ...
            response = client.post("/tasks/quick/bulk-enrichment", json=task_data, headers=auth_headers)
    ...
    def test_admin_can_create_database_cleanup(self, client, admin_user, auth_headers):
        ...
        with (
            patch("MakerMatrix.auth.dependencies.auth.get_current_user", return_value=admin_user),
            patch("MakerMatrix.routers.task_routes.require_permission") as mock_require_permission,
            patch(
                "MakerMatrix.services.task_security_service.task_security_service.validate_task_creation"
            ) as mock_validate,
            ...
            task_data = {"cleanup_options": {"remove_old_tasks": True, "optimize_database": True}}
    ...
    def test_concurrent_task_limiting(self, client, regular_user, auth_headers):
        ...
        with (
            ...
            mock_validate.return_value = (
                False,
                "Too many concurrent PART_ENRICHMENT tasks (2/2). Wait for existing tasks to complete.",
            )
    ...
    def test_security_permissions_endpoint(self, client, regular_user, auth_headers):
        ...
        with (
            ...
            patch("MakerMatrix.routers.task_routes.require_permission") as mock_require_permission,
            patch(
                "MakerMatrix.services.task_security_service.task_security_service._get_user_permissions"
            ) as mock_get_perms,
        ):
    ...
    def test_task_validation_endpoint(self, client, regular_user, auth_headers):
        ...
        with (
            ...
            response = client.post("/tasks/security/validate", json=validation_request, headers=auth_headers)
    ...
    def test_viewer_user_permissions(self, client, viewer_user, auth_headers):
        ...
        with (
            patch("MakerMatrix.auth.dependencies.auth.get_current_user", return_value=viewer_user),
            patch("MakerMatrix.routers.task_routes.require_permission") as mock_require_permission,
            patch(
                "MakerMatrix.services.task_security_service.task_security_service.validate_task_creation"
            ) as mock_validate,
    ...
    def test_audit_logging_on_security_denial(self, client, regular_user, auth_headers):
        ...
        with (
            ...
            patch(
                "MakerMatrix.services.task_security_service.task_security_service.log_task_security_event"
            ) as mock_log_event,
        ):
    ...
    def test_worker_management_security(self, client, regular_user, admin_user, auth_headers):
        ...
        with (
            ...
            mock_require_permission.return_value = lambda: exec(
