...
class TestLocationImageSupport:
    ...
    def setup(self):
        ...
        login_response = self.client.post("/auth/login", data={"username": "admin", "password": "Admin123!"})
    ...
    def test_create_location_with_image_url(self):
        ...
        response = self.client.post("/locations/add_location", json=location_data, headers=self.headers)
    ...
    def test_create_location_without_image_url(self):
        ...
        location_data = {"name": "Plain Location", "description": "Location without image", "location_type": "shelf"}
    ...
    def test_update_location_add_image_url(self):
        ...
        location_data = {"name": "Location to Update", "description": "Will add image later", "location_type": "drawer"}
        ...
        response = self.client.put(f"/locations/update_location/{location_id}", json=update_data, headers=self.headers)
    ...
    def test_get_location_includes_image_url(self):
        ...
        # Get location by ID
        response = self.client.get(f"/locations/get_location?location_id={location_id}", headers=self.headers)
    ...
    def test_get_all_locations_includes_image_urls(self):
        ...
        # Get all locations
        response = self.client.get("/locations/get_all_locations", headers=self.headers)
    ...
    def test_location_hierarchy_with_images(self):
        ...
        response = self.client.post("/locations/add_location", json=parent_data, headers=self.headers)
        ...
        response = self.client.post("/locations/add_location", json=child_data, headers=self.headers)

        assert response.status_code == 200
        child_id = response.json()["data"]["id"]

        # Get location details for parent (includes children)
        response = self.client.get(f"/locations/get_location_details/{parent_id}", headers=self.headers)
