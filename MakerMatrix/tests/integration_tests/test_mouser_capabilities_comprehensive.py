...
class TestMouserCapabilities:
    ...
    async def test_search_parts_capability(self, mouser_supplier, test_part_number):
        ...
        assert any(test_part_number in pn for pn in part_numbers), f"Test part {test_part_number} should be in search results"
    ...
    async def test_get_part_details_capability(self, mouser_supplier, test_part_number):
        ...
        assert any(term in part_details.description.lower() for term in ['optocoupler', 'opto', 'coupler', 'isolator']), \
    ...
    async def test_fetch_specifications_capability(self, mouser_supplier, test_part_number):
        ...
        if specs is not None:
            ...
            if len(specs) > 0:
                ...
                found_specs = [spec_type for spec_type in expected_spec_types if any(spec_type in key for key in spec_keys)]
    ...
    async def test_parametric_search_capability(self, mouser_supplier):
        ...
        relevant_results = [desc for desc in descriptions if any(term in desc for term in ['opto', 'coupler', 'isolat'])]
        
        # At least half the results should be relevant
        relevance_ratio = len(relevant_results) / len(descriptions) if descriptions else 0
        assert relevance_ratio > 0.3, f"Search results should be relevant to optocouplers. Relevance: {relevance_ratio:.2f}"
