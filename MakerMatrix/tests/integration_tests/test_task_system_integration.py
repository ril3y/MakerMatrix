...
class TestTaskSystemIntegration:
    ...
    async def test_task_lifecycle_integration(self, clean_tasks):
        ...
        update_request = UpdateTaskRequest(
            ...
            current_step="Halfway through cleanup"
        )
    ...
    async def test_task_filtering_integration(self, clean_tasks):
        ...
        task1 = await service.create_task(CreateTaskRequest(
            task_type=TaskType.CSV_ENRICHMENT,
            name="High Priority Enrichment",
            priority=TaskPriority.HIGH
        ), user_id="user1")
        tasks.append(task1)
        
        # Normal priority price update task
        task2 = await service.create_task(CreateTaskRequest(
        ...
        task3 = await service.create_task(CreateTaskRequest(
            task_type=TaskType.DATABASE_CLEANUP,
        ...
        filter_request = TaskFilterRequest(
            task_type=[TaskType.CSV_ENRICHMENT, TaskType.PRICE_UPDATE],
            limit=10
        )
    ...
    async def test_task_retry_integration(self, clean_tasks):
        ...
        task = await service.create_task(CreateTaskRequest(
        ...
        # Mark it as failed
        await service.update_task(task.id, UpdateTaskRequest(
...
class TestCSVEnrichmentTaskIntegration:
    ...
    async def test_task_dependencies(self, clean_tasks):
        ...
        parent_task = await service.create_task(CreateTaskRequest(
            task_type=TaskType.CSV_ENRICHMENT,
            name="Parent Enrichment Task"
        ))
        
        # Create dependent tasks
        child_task1 = await service.create_task(CreateTaskRequest(
        ...
        child_task2 = await service.create_task(CreateTaskRequest(
            task_type=TaskType.PART_VALIDATION,
            name="Validate Parts",
            parent_task_id=parent_task.id,
            depends_on_task_ids=[parent_task.id]
    ...
    async def test_scheduled_tasks(self, clean_tasks):
        ...
        future_task = await service.create_task(CreateTaskRequest(
            task_type=TaskType.PRICE_UPDATE,
        ...
        past_task = await service.create_task(CreateTaskRequest(
            task_type=TaskType.DATABASE_CLEANUP,
            name="Past Scheduled Cleanup",
