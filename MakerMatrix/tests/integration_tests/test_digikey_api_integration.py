...
class TestDigiKeyConfiguration:
    ...
    def test_test_connection_not_configured(self, client, auth_headers):
        ...
        config_data = {"credentials": {}, "config": {}}

        response = client.post("/api/suppliers/digikey/test", json=config_data, headers=auth_headers)
        ...
        if data["status"] == "error":
            ...
            assert (
                ...
                or "credentials" in data["message"].lower()
                or "not configured" in data["message"].lower()
            )
    ...
    def test_test_connection_with_credentials(self, client, auth_headers, digikey_config):
        ...
        with patch("requests.get") as mock_get:
            ...
            response = client.post("/api/suppliers/digikey/test", json=digikey_config, headers=auth_headers)
...
class TestDigiKeyOAuthFlow:
    ...
    def test_get_oauth_authorization_url(self, client, auth_headers, digikey_config):
        ...
        response = client.post(
            "/api/suppliers/digikey/oauth/authorization-url", json=digikey_config, headers=auth_headers
        )
    ...
    def test_oauth_callback_success(self, client):
        ...
        response = client.get("/api/suppliers/digikey/oauth/callback?code=test_auth_code")
    ...
    def test_oauth_callback_error(self, client):
        ...
        response = client.get("/api/suppliers/digikey/oauth/callback?error=access_denied")
...
class TestDigiKeyPartSearch:
    ...
    def test_search_parts(self, mock_digikey, client, auth_headers, digikey_config):
        ...
        with patch("MakerMatrix.suppliers.registry.SupplierRegistry.get_supplier") as mock_get_supplier:
            ...
            search_request = {"query": "test resistor", "limit": 10}

            response = client.post(
                "/api/suppliers/digikey/search", json={**digikey_config, **search_request}, headers=auth_headers
            )
...
TEST456-DK,Another Mfg,MANU789,Test Capacitor 10uF,5,$0.25"""
    ...
    async def test_digikey_csv_import_direct(self):
        ...
        result = await supplier.import_order_file(self.test_csv_content.encode("utf-8"), "csv", "test_digikey.csv")
