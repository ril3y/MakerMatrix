...
def mock_supplier_with_pricing():
    ...
    supplier.fetch_pricing = AsyncMock(return_value=[
...
class TestPriceUpdateTask:
    ...
    async def test_execute_with_pricing_capable_suppliers(self, mock_task, mock_parts, mock_supplier_with_pricing):
        ...
        with patch('MakerMatrix.tasks.price_update_task.get_session') as mock_get_session, \
    ...
    @pytest.mark.asyncio
    async def test_skips_suppliers_without_pricing_capability(self, mock_task, mock_parts, mock_supplier_without_pricing):
...
def test_supplier_capability_checking():
    ...
    datasheet_only_supplier.get_capabilities.return_value = [
        SupplierCapability.FETCH_DATASHEET
    ]
