...
def enrichment_queue_manager(memory_engine, mock_rate_limit_service, mock_websocket_manager):
    ...
    with patch("MakerMatrix.services.system.enrichment_queue_manager.get_available_suppliers") as mock_suppliers:
        ...
        manager = EnrichmentQueueManager(memory_engine, mock_rate_limit_service, mock_websocket_manager)
        return manager
...
class TestEnrichmentQueueManager:
    ...
    async def test_get_queue_statistics(self, enrichment_queue_manager):
        ...
        await enrichment_queue_manager.queue_part_enrichment(
            part_id="part-1", part_name="Part 1", supplier_name="MOUSER", capabilities=["fetch_datasheet"]
        )
        await enrichment_queue_manager.queue_part_enrichment(
            part_id="part-2", part_name="Part 2", supplier_name="LCSC", capabilities=["fetch_image"]
        )
    ...
    async def test_websocket_broadcast_on_queue(self, enrichment_queue_manager, mock_websocket_manager):
        ...
        await enrichment_queue_manager.queue_part_enrichment(
            part_id="part-ws", part_name="WebSocket Test Part", supplier_name="MOUSER", capabilities=["fetch_datasheet"]
        )
...
async def test_rate_limit_service_integration(enrichment_queue_manager, mock_rate_limit_service):
    ...
    await enrichment_queue_manager.queue_part_enrichment(
        part_id="part-rate-limit", part_name="Rate Limit Test", supplier_name="MOUSER", capabilities=["fetch_datasheet"]
    )
