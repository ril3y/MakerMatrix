...
def test_supplier_name_field_mapping():
    ...
    mock_api_response = {
        "status": "success",
        "data": [
            {"id": "digikey", "name": "DigiKey Electronics", "configured": True, "enabled": True},
            {"id": "lcsc", "name": "LCSC Electronics", "configured": False, "enabled": False},
        ],
    }
    ...
    def extract_supplier_names(response_data):
        ...
        return set(
            ...
            and [(s.get("name") or s.get("supplier_name") or s.get("id") or "").upper() for s in response_data["data"]]
            or []
        )
