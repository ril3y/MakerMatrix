...
class TestLocationCreateRequestSchema:
    ...
    def test_regular_location_request(self):
        ...
        request = LocationCreateRequest(name="Workshop", description="Main workshop area", location_type="building")
    ...
    def test_simple_container_request(self):
        ...
        request = LocationCreateRequest(
            name="32-Slot Box", location_type="container", slot_count=32, slot_naming_pattern="Slot {n}"
        )
    ...
    def test_slot_count_validation_minimum(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            LocationCreateRequest(name="Invalid Container", slot_count=0)  # Should fail: minimum is 1
        assert "greater than or equal to 1" in str(exc_info.value).lower()

    def test_slot_count_validation_maximum(self):
        """Test slot_count maximum value validation (le=200)."""
        with pytest.raises(ValueError) as exc_info:
            LocationCreateRequest(name="Invalid Container", slot_count=201)  # Should fail: maximum is 200
    ...
    def test_custom_layout_field(self):
        ...
        request = LocationCreateRequest(
            name="Custom Container", slot_count=10, slot_layout_type="custom", slot_layout={"custom_data": "test"}
        )
...
class TestAddLocationEndpointLogic:
    ...
    def test_regular_location_routing_logic(self):
        ...
        location_data = LocationCreateRequest(name="Workshop", description="Test workshop", location_type="building")
    ...
    def test_container_with_zero_slots_routing(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            LocationCreateRequest(name="Container", slot_count=0)  # Should fail: minimum is 1
        assert "greater than or equal to 1" in str(exc_info.value).lower()
...
class TestBackwardCompatibility:
    ...
    def test_existing_location_creation_unchanged(self):
        ...
        request = LocationCreateRequest(name="Warehouse", description="Storage area", location_type="building")
