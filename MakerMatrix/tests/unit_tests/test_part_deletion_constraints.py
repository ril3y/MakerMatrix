...
class TestPartDeletionConstraints:
    ...
    def sample_order_item(self, sample_part):
        ...
        order = OrderModel(
            order_number="TEST-ORDER-001", supplier="TestSupplier", order_date=datetime(2025, 6, 15), status="delivered"
        )
    ...
    def test_delete_part_with_order_items_succeeds_with_fix(self, in_memory_db, sample_part, sample_order_item):
        ...
        with Session(in_memory_db) as session:
            ...
            remaining_item = session.exec(select(OrderItemModel).where(OrderItemModel.id == order_item_id)).first()
    ...
    def test_part_service_delete_part_handles_constraints(self, sample_part):
        ...
        with (
            patch.object(PartRepository, "get_part_by_id") as mock_get,
    ...
    def test_cascade_delete_configuration_exists(self):
        ...
        assert ondelete_action in [
            "CASCADE",
            "SET NULL",
        ], f"Foreign key should have CASCADE or SET NULL, got: {ondelete_action}"
    ...
    def test_repository_delete_with_cleanup_logic(self, in_memory_db, sample_part, sample_order_summary):
        ...
        with Session(in_memory_db) as session:
            ...
            try:
                ...
                remaining_summaries = (
                    session.query(PartOrderSummary).filter(PartOrderSummary.part_id == sample_part.id).all()
                )
    ...
    def test_bulk_part_deletion_with_dependencies(self, in_memory_db):
        ...
        with Session(in_memory_db) as session:
            ...
            for i in range(3):
                part = PartModel(part_number=f"BULK-{i}", part_name=f"Bulk Test Part {i}", quantity=10 + i)
                session.add(part)
                session.commit()
        ...
        try:
            ...
            if "FOREIGN KEY constraint failed" in str(mock_error):
                user_friendly_message = (
                    "Cannot delete part because it has associated order data. Please remove order history first."
                )
    ...
    @pytest.mark.parametrize(
        "constraint_type,expected_message",
        [
