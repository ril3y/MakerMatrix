...
class TestUserAuthenticationAuthorization:
    ...
    def create_test_users(self) -> Dict[str, UserModel]:
        ...
        users = {"admin": admin_user, "regular": regular_user, "readonly": readonly_user, "inactive": inactive_user}
    ...
    def create_test_roles(self) -> Dict[str, RoleModel]:
        ...
        readonly_role = RoleModel(
            ...
            permissions=["parts:read", "locations:read", "categories:read", "tasks:read"],
        )

        roles = {"admin": admin_role, "regular": regular_role, "readonly": readonly_role}
    ...
    def test_user_authentication_success(self):
        ...
        # Test admin login
        admin_auth = self.auth_service.authenticate_user(session, "admin_test", "admin123")
        assert admin_auth is not None
        assert admin_auth.username == "admin_test"
        assert admin_auth.is_active == True

        # Test regular user login
        regular_auth = self.auth_service.authenticate_user(session, "regular_test", "user123")
        ...
        readonly_auth = self.auth_service.authenticate_user(session, "readonly_test", "readonly123")
    ...
    def test_user_authentication_failure(self):
        ...
        # Test wrong password
        wrong_password = self.auth_service.authenticate_user(session, "admin_test", "wrongpassword")
        assert wrong_password is None

        # Test non-existent user
        non_existent = self.auth_service.authenticate_user(session, "nonexistent", "password")
        assert non_existent is None

        # Test inactive user
        inactive_auth = self.auth_service.authenticate_user(session, "inactive_test", "inactive123")
        assert inactive_auth is None  # Should fail due to inactive status

        # Test empty credentials
        empty_username = self.auth_service.authenticate_user(session, "", "password")
        assert empty_username is None

        empty_password = self.auth_service.authenticate_user(session, "admin_test", "")
    ...
    def test_jwt_token_operations(self):
        ...
        # Test token creation
        user_data = {"user_id": "admin-user-001", "username": "admin_test", "roles": ["admin"]}
    ...
    def test_role_based_access_control(self):
        ...
        test_location = LocationModel(
            id="test-location-001", name="Test Location", description="Location for permission testing"
        )
        ...
        # Admin can read
        can_read = self.permission_service.has_permission(admin_user, "parts:read")
        assert can_read == True

        # Admin can write
        can_write = self.permission_service.has_permission(admin_user, "parts:write")
        assert can_write == True

        # Admin can delete
        can_delete = self.permission_service.has_permission(admin_user, "parts:delete")
        ...
        can_read = self.permission_service.has_permission(regular_user, "parts:read")
        assert can_read == True

        # Regular user can write
        can_write = self.permission_service.has_permission(regular_user, "parts:write")
        assert can_write == True

        # Regular user cannot delete
        can_delete = self.permission_service.has_permission(regular_user, "parts:delete")
        ...
        can_read = self.permission_service.has_permission(readonly_user, "parts:read")
        assert can_read == True

        # Readonly user cannot write
        can_write = self.permission_service.has_permission(readonly_user, "parts:write")
        assert can_write == False

        # Readonly user cannot delete
        can_delete = self.permission_service.has_permission(readonly_user, "parts:delete")
        assert can_delete == False
    ...
    def test_permission_enforcement_operations(self):
        ...
        test_location = LocationModel(
            id="test-location-perm", name="Permission Test Location", description="Location for permission testing"
        )
        session.add(test_location)
        session.commit()
        ...
        readonly_user = self.test_users["readonly"]
        cannot_create = not self.permission_service.has_permission(readonly_user, "parts:write")
        ...
        cannot_delete = not self.permission_service.has_permission(regular_user, "parts:delete")
        assert cannot_delete == True

        # Readonly user cannot delete
        cannot_delete = not self.permission_service.has_permission(readonly_user, "parts:delete")
        assert cannot_delete == True
    ...
    def test_user_management_permissions(self):
        ...
        can_manage_users = self.permission_service.has_permission(admin_user, "users:write")
        assert can_manage_users == True

        # Regular user cannot manage users
        cannot_manage_users = not self.permission_service.has_permission(regular_user, "users:write")
        assert cannot_manage_users == True

        # Readonly user cannot manage users
        cannot_manage_users = not self.permission_service.has_permission(readonly_user, "users:write")
        assert cannot_manage_users == True
        ...
        for operation in admin_operations:
            ...
            admin_has_perm = self.permission_service.has_permission(admin_user, operation)
            assert admin_has_perm == True

            # Regular user does not have admin permissions
            regular_no_perm = not self.permission_service.has_permission(regular_user, operation)
            assert regular_no_perm == True

            # Readonly user does not have admin permissions
            readonly_no_perm = not self.permission_service.has_permission(readonly_user, operation)
    ...
    def test_task_management_permissions(self):
        ...
        can_create_tasks = self.permission_service.has_permission(regular_user, "tasks:write")
        assert can_create_tasks == True

        # Test task deletion permissions (admin only)
        can_delete_tasks = self.permission_service.has_permission(admin_user, "tasks:delete")
        assert can_delete_tasks == True

        cannot_delete_tasks = not self.permission_service.has_permission(regular_user, "tasks:delete")
        assert cannot_delete_tasks == True

        # Test task reading permissions (all users)
        can_read_tasks = self.permission_service.has_permission(readonly_user, "tasks:read")
        assert can_read_tasks == True

        # Test task modification permissions
        cannot_modify_tasks = not self.permission_service.has_permission(readonly_user, "tasks:write")
    ...
    def test_password_management(self):
        ...
        auth_old = self.auth_service.authenticate_user(session, regular_user.username, old_password)
        assert auth_old is not None

        # Change password
        password_changed = self.auth_service.change_password(session, regular_user.id, old_password, new_password)
        assert password_changed == True

        # Verify new password works
        auth_new = self.auth_service.authenticate_user(session, regular_user.username, new_password)
        assert auth_new is not None

        # Verify old password no longer works
        auth_old_fail = self.auth_service.authenticate_user(session, regular_user.username, old_password)
    ...
    def test_role_assignment_operations(self):
        ...
        role_assigned = self.role_service.assign_role_to_user(session, test_user.id, regular_role.id)
        ...
        additional_role = self.role_service.assign_role_to_user(session, test_user.id, readonly_role.id)
        ...
        # Test removing role
        role_removed = self.role_service.remove_role_from_user(session, test_user.id, readonly_role.id)
    ...
    def test_permission_inheritance(self):
        ...
        user_permissions = self.permission_service.get_user_permissions(multi_role_user)
        ...
        can_read = self.permission_service.has_permission(multi_role_user, "parts:read")
        assert can_read == True

        can_write = self.permission_service.has_permission(multi_role_user, "parts:write")
        assert can_write == True

        cannot_delete = not self.permission_service.has_permission(multi_role_user, "parts:delete")
        assert cannot_delete == True
    ...
    def test_security_audit_logging(self):
        ...
        auth_attempt = self.auth_service.authenticate_user(session, "admin_test", "admin123")
        ...
        regular_user = self.test_users["regular"]
        permission_denied = not self.permission_service.has_permission(regular_user, "parts:delete")
