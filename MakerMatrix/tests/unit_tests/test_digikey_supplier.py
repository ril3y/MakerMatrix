...
from MakerMatrix.suppliers.exceptions import SupplierConfigurationError, SupplierAuthenticationError
...
class TestDigiKeyAuthentication:
    ...
    async def test_authentication_missing_client_id(self):
        ...
        self.supplier.configure(credentials={"client_secret": "test_secret"}, config={})
    ...
    async def test_authentication_missing_client_secret(self):
        ...
        self.supplier.configure(credentials={"client_id": "test_client_id"}, config={})
    ...
    async def test_authentication_missing_library(self):
        ...
        self.supplier.configure(credentials={"client_id": "test_client_id", "client_secret": "test_secret"}, config={})
...
class TestDigiKeyCSVImport:
    ...
    def test_can_import_file_with_content(self):
        ...
        digikey_csv_content = (
            b"Digi-Key Part Number,Manufacturer Part Number,Description,Quantity\nTEST123,MANU456,Test Part,5"
        )
...
Value1,Value2,Value3"""
    ...
    async def test_import_excel_file(self, mock_read_excel):
        ...
        mock_df = pd.DataFrame({"Digi-Key Part Number": ["TEST123"], "Description": ["Test Part"], "Quantity": [5]})
...
class TestDigiKeyTokenManagement:
    ...
    async def test_token_refresh(self):
        ...
        with patch("aiohttp.ClientSession.post") as mock_post:
            ...
            mock_response = AsyncMock()
            mock_response.status = 200
            mock_response.json = AsyncMock(return_value={"access_token": "new_access_token", "expires_in": 3600})
