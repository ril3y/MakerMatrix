...
class TestLCSCEnrichmentFix:
    ...
    def cleanup_test_part(self):
        ...
        try:
            with Session(engine) as session:
                part = session.exec(select(PartModel).where(PartModel.part_number == "C2918489")).first()
                if part:
                    session.delete(part)
                    session.commit()
    ...
    async def test_lcsc_enrichment_with_specifications(self, enrichment_handlers, cleanup_test_part):
        ...
        with Session(engine) as session:
            ...
            existing_part = session.exec(select(PartModel).where(PartModel.part_number == "C2918489")).first()
