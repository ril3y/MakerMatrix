...
class ToolCreateRequest(BaseModel):
    ...
    tool_type: Optional[str] = Field(
        None, max_length=100, description="Tool category: hand_tool, power_tool, measuring_instrument, consumable, etc."
    )
    ...
    additional_properties: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Tool-specific properties as flat key-value pairs"
    )

    condition: Optional[str] = Field(
        "good", description="Tool condition: excellent, good, fair, poor, needs_repair, out_of_service"
    )
...
class ToolMaintenanceRequest(BaseModel):
    ...
    maintenance_type: str = Field(..., description="Type of maintenance: calibration, repair, inspection, cleaning")
    performed_by: Optional[str] = Field(
        None, description="User ID who performed maintenance (auto-set by backend if not provided)"
    )
