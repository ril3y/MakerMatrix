...
class UnifiedColumnMapper:
    ...
    STANDARD_MAPPINGS = {
        'part_number': [
            'part #', 'part number', 'supplier part', 'supplier part number',
            'lcsc part number', 'mouser #:', 'mouser part #', 'mouser part number',
            'mouser p/n', 'digikey part number', 'digi-key part number'
        ],
        'manufacturer': [
            'manufacturer', 'mfr', 'mfg', 'brand', 'manufacture'
        ],
        'manufacturer_part_number': [
            'mfr part #', 'manufacturer part number', 'mpn', 'mfr. #:', 
            'mfg part #', 'customer #', 'manufacture part number'
        ],
        'description': [
            'description', 'desc', 'product description', 'desc.:'
        ],
        'quantity': [
            'quantity', 'qty', 'order qty', 'order qty.', 'order quantity'
        ],
        'unit_price': [
            'unit price', 'price', 'unit cost', 'price (usd)', 'unit price($)'
        ],
        'order_price': [
            'order price', 'total price', 'line total', 'ext. (usd)', 
            'extended price', 'order price($)'
        ],
        'package': [
            'package', 'packaging', 'case', 'footprint', 'case/package'
        ],
        'rohs': [
            'rohs', 'rohs status', 'rohs compliant', 'rohs compliance'
        ],
        'customer_reference': [
            'customer no.', 'customer #', 'customer reference', 'customer part number'
        ],
        'min_order_qty': [
            'min order qty', 'minimum order', 'min qty', 'min\\mult order qty.'
        ]
    }
    ...
    def map_columns(self, df_columns: List[str], supplier_mappings: Optional[Dict] = None) -> Dict[str, str]:
        ...
        for field, possible_names in all_mappings.items():
            for possible_name in possible_names:
                ...
                matching_cols = [
                    original_col for lower_col, original_col in lower_columns.items()
