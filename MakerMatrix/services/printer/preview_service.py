...
from MakerMatrix.printers.base import PreviewResult, LabelSize, PrinterInterface, InvalidLabelSizeError
...
class PreviewService:
    ...
    async def preview_part_qr_code(
        self, part: PartModel, label_size: str = "12", printer: Optional[PrinterInterface] = None
    ) -> PreviewResult:
        ...
        print_settings = PrintSettings(
            label_size=label_height_mm, dpi=300, qr_scale=0.95, qr_min_size_mm=8.0, qr_max_margin_mm=1.0
        )

        part_dict = {"id": part.id}
        qr_image = LabelService.generate_optimized_qr(part_dict, print_settings)
        print(
            f"[DEBUG] Preview QR code: Generated optimized QR code for part {part.id}, size: {qr_image.width}x{qr_image.height}"
        )

        # Generate preview using the printer's preview method
        return await printer_to_use.preview_label(qr_image, label_size)

    async def preview_part_name(
        self, part: PartModel, label_size: str = "12", printer: Optional[PrinterInterface] = None
    ) -> PreviewResult:
    ...
    async def preview_text_label(
        self, text: str, label_size: str = "12", printer: Optional[PrinterInterface] = None
    ) -> PreviewResult:
    ...
    async def preview_combined_label(
        ...
        print(
            f"[DEBUG] Preview: QR sizing - label {label_length_mm}mm, max QR {max_qr_size_mm:.1f}mm, min QR {qr_min_size_mm:.1f}mm, scale {qr_scale_factor:.2f}"
        )
        ...
        qr_image = LabelService.generate_optimized_qr(part_dict, print_settings)
        print(
            f"[DEBUG] Preview: Generated optimized QR code for combined layout, size: {qr_image.width}x{qr_image.height}"
        )
        ...
        print(
            f"[DEBUG] Preview: Label dimensions: {label_height_mm}mm x {label_length or 'auto'}mm = {label_width_px}x{label_height_px}px"
        )
    ...
    def _create_combined_image(
        self, qr_image: Image.Image, text: str, size: tuple[int, int], options: Optional[dict] = None
    ) -> Image.Image:
        ...
        if emoji_value:
            ...
            try:
                ...
                print(
                    f"[DEBUG] Preview: Emoji rendered at ({current_x - emoji_img.width - 10}, {emoji_y}), size: {emoji_img.width}x{emoji_img.height}"
                )
        ...
        if font_size_override is not None:
            ...
            if is_single_line and size[1] <= 200:  # Assume narrow label like 12mm
                ...
                while font_size > 8:
                    ...
                    print(
                        f"[DEBUG] Preview: Testing font_size={font_size}: text_width={test_width}px, limit={max_text_width}px"
                    )
...
class PreviewManager:
    ...
    async def preview_with_printer(self, printer_id: str, image: Image.Image, label_size: str) -> PreviewResult:
