...
"""
    ...
    def _get_sql_engine(self):
        """Get or create SQLAlchemy engine for LangChain"""
    ...
    def _get_sql_chain(self, config: AIConfig):
        """Get or create LangChain SQL Database Chain with Ollama LLM"""
        if not LANGCHAIN_AVAILABLE:
            return None
        if not self._sql_chain and config.provider.lower() == "ollama":
            try:
                ...
                if sql_db:
                    ...
                    self._sql_chain = SQLDatabaseChain.from_llm(
                        ...
                        return_sql=True  # Include SQL in the response
                    )
    ...
    async def _try_langchain_sql_query(self, message: str, config: AIConfig) -> Optional[Dict[str, Any]]:
        """Try to answer the question using LangChain SQL Database Chain"""
        if not LANGCHAIN_AVAILABLE:
            return None
        try:
            # Check if this looks like a database query question
            query_keywords = ['how many', 'find', 'search', 'list', 'show', 'count', 'what', 'where', 'inventory', 'parts', 'categories']
            if not any(keyword.lower() in message.lower() for keyword in query_keywords):
                return None
            
            # Get the SQL chain
            sql_chain = self._get_sql_chain(config)
            if not sql_chain:
            ...
            # The result should contain both the SQL query and the natural language answer
            return {
                "success": True,
                "response": result,
                "model": config.model_name,
    ...
    async def chat_with_ai(self, message: str, conversation_history: List[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            # Try LangChain SQL Database Chain for Ollama first
            if config.provider.lower() == "ollama":
                langchain_response = await self._try_langchain_sql_query(message, config)
                if langchain_response:
                    return langchain_response
            
            # Fallback to original chat method for other providers or if LangChain fails
