...
class EnhancedImportService:
    ...
    async def import_csv_with_enrichment(
        ...
        try:
            ...
            if not import_result["success"]:
                await self._broadcast_message(
                    create_notification_message(title="Import Failed", message=import_result["message"], level="error")
                )
            ...
            if enrichment_enabled and total_parts > 0:
                ...
                enrichment_results = await self._queue_enrichment_tasks(
                    imported_parts, import_result["data"]["parser_type"], enrichment_priority, import_id
                )
            ...
            await self._broadcast_message(
                create_notification_message(title="Import Error", message=f"Import failed: {str(e)}", level="error")
            )
    ...
    async def _import_csv_data(
        ...
        try:
            ...
            if not import_result.parts:
                ...
                return {"success": False, "message": "No parts found in file", "data": {}}
            ...
            for part_data in import_result.parts:
                try:
                    ...
                    if result.get("success"):
                        ...
                        failed_items.append({"part_data": part_data, "error": result.get("message", "Unknown error")})
                except Exception as e:
                    failed_items.append({"part_data": part_data, "error": str(e)})
            ...
            return {"success": False, "message": f"File import failed: {str(e)}", "data": {}}

    async def _queue_enrichment_tasks(
        self, imported_parts: List[Dict[str, Any]], parser_type: str, priority: EnrichmentPriority, import_id: str
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not supports_enrichment:
                logger.info(
                    f"Parser type '{parser_type}' does not support enrichment - supplier not configured or enabled"
                )
        ...
        logger.info(
            f"Queued {len([r for r in enrichment_results if r['status'] == 'queued'])} enrichment tasks for import {import_id}"
        )
        ...
        try:
            ...
            for i, part_id in enumerate(part_ids):
                try:
                    ...
                    results.append({"part_id": part_id, "task_id": None, "status": "failed", "error": str(e)})
            ...
            await self._broadcast_message(
                create_notification_message(
                    title="Enrichment Error", message=f"Bulk enrichment failed: {str(e)}", level="error"
                )
            )
    ...
    async def get_enrichment_queue_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "message": f"Failed to get queue status: {str(e)}"}
        ...
        try:
            ...
            for task_id in task_ids:
                try:
                    ...
                    results.append(
                        {"task_id": task_id, "cancelled": success, "status": "cancelled" if success else "not_found"}
                    )
                except Exception as e:
                    results.append({"task_id": task_id, "cancelled": False, "status": "error", "error": str(e)})
            ...
            return {
                ...
                "data": {"cancelled_count": cancelled_count, "total_requested": len(task_ids), "results": results},
            }

        except Exception as e:
            logger.error(f"Failed to cancel tasks: {e}")
            return {"success": False, "message": f"Failed to cancel tasks: {str(e)}"}
