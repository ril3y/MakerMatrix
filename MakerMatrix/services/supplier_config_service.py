...
from MakerMatrix.services.encryption_service import EncryptionService, CredentialEncryptionMixin
...
class SupplierConfigService(CredentialEncryptionMixin):
    """
    ...
    with automatic encryption/decryption of sensitive data.
    """
    
    def __init__(self):
        self.encryption_service = self.get_encryption_service()
    ...
    def get_supplier_config(self, supplier_name: str, include_credentials: bool = False) -> SupplierConfigModel:
        ...
        """
        with Session(engine) as session:
            query = session.query(SupplierConfigModel).filter(
                SupplierConfigModel.supplier_name == supplier_name
            )
            
            if include_credentials:
                query = query.options(joinedload(SupplierConfigModel.credentials))
    ...
    def get_all_supplier_configs(self, enabled_only: bool = False) -> List[Dict[str, Any]]:
        ...
        """
        with Session(engine) as session:
            query = session.query(SupplierConfigModel).options(
                joinedload(SupplierConfigModel.credentials)
            )
    ...
    def set_supplier_credentials(self, supplier_name: str, credentials: Dict[str, str], 
                               user_id: Optional[str] = None) -> SupplierCredentialsModel:
        """
        Set encrypted credentials for a supplier
        
        Args:
            supplier_name: Name of the supplier
            credentials: Dictionary of credential fields (api_key, secret_key, etc.)
            user_id: ID of the user setting credentials
            
        Returns:
            Created or updated credentials model
        ...
        with Session(engine) as session:
            ...
            # Encrypt credentials
            encrypted_creds, salt = self.encrypt_credentials_dict(credentials)
            key_id = self.encryption_service.generate_key_id(salt)
            
            # Check if credentials already exist
            existing_creds = session.query(SupplierCredentialsModel).filter(
                SupplierCredentialsModel.supplier_config_id == config.id
            ).first()
            
            if existing_creds:
                # Update existing credentials
                for field, value in encrypted_creds.items():
                    setattr(existing_creds, field, value)
                
                existing_creds.encryption_key_id = key_id
                existing_creds.salt = salt
                existing_creds.updated_at = datetime.utcnow()
                existing_creds.last_rotated_at = datetime.utcnow()
                
                session.commit()
                session.refresh(existing_creds)
                
                self.logger.info(f"Updated credentials for supplier: {supplier_name}")
                return existing_creds
            else:
                # Create new credentials
                creds_model = SupplierCredentialsModel(
                ...
                for field, value in encrypted_creds.items():
                    setattr(creds_model, field, value)
                
                session.add(creds_model)
                session.commit()
                session.refresh(creds_model)
                
                self.logger.info(f"Created credentials for supplier: {supplier_name}")
                return creds_model
    ...
    def get_supplier_credentials(self, supplier_name: str, decrypt: bool = True) -> Optional[Dict[str, str]]:
        ...
        with Session(engine) as session:
            ...
            encrypted_fields = {
                ...
                'username_encrypted': creds.username_encrypted,
                'password_encrypted': creds.password_encrypted,
                'oauth_token_encrypted': creds.oauth_token_encrypted,
                'refresh_token_encrypted': creds.refresh_token_encrypted
            }
    ...
    def export_supplier_configs(self, include_credentials: bool = False) -> Dict[str, Any]:
        ...
        for config in configs:
            ...
            if include_credentials and config.credentials:
                config_dict["credentials"] = config.credentials.to_dict(include_encrypted=True)
        ...
        for supplier_data in import_data.get("suppliers", []):
            try:
                ...
                config_data = {k: v for k, v in supplier_data.items() 
                             if k not in ["id", "created_at", "updated_at", "credentials", "has_credentials"]}
