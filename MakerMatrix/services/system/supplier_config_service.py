...
class SupplierConfigService(BaseService):
    ...
    def set_supplier_credentials(
        self, supplier_name: str, credentials: Dict[str, str], user_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_supplier_credentials(self, supplier_name: str) -> Optional[Dict[str, str]]:
        ...
        try:
            with self.get_session() as session:
                ...
                credentials_model = self.credentials_repo.get_credentials_by_supplier_name(session, supplier_name)
            ...
            try:
                ...
                with self.get_session() as session:
                    self.supplier_config_repo.update_test_status(
                        session, supplier_name, "success" if success else "failed", start_time
                    )
