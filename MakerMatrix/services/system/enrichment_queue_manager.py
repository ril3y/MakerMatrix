        ...
        try:
            ...
            for supplier_name in available_suppliers:
                self.supplier_queues[supplier_name.upper()] = SupplierQueue(
                    supplier_name.upper(), self.rate_limit_service
                )
    ...
    async def _process_enrichment_task(self, task: EnrichmentTask, queue: SupplierQueue):
        ...
        try:
            ...
            for capability in task.remaining_capabilities:
                ...
                # Check rate limit
                rate_status = await self.rate_limit_service.check_rate_limit(task.supplier_name, capability)

                if not rate_status.get("allowed", False):
                    raise RateLimitExceeded(
                        task.supplier_name, "rate_limit", rate_status.get("retry_after_seconds", 60)
                    )
                ...
                try:
                    ...
                    await self.rate_limit_service.record_request(task.supplier_name, capability, True, response_time)
                    ...
                    await self.rate_limit_service.record_request(
                        task.supplier_name, capability, False, response_time, str(e)
                    )
                ...
                try:
                    ...
                    if success_count == total_count:
                        message = create_toast_message(
                            "success", f"✅ Enrichment completed for {task.part_name}", duration=3000
                        )
                        ...
                        message = create_toast_message(
                            "error", f"❌ Enrichment failed for {task.part_name}", duration=5000
                        )
