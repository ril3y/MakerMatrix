...
class PartEnrichmentService(BaseService):
    ...
    async def handle_part_enrichment(
        ...
        try:
            ...
            logger.info(
                f"[ENRICHMENT] Starting enrichment for part_id={part_id}, supplier={preferred_supplier}, capabilities={requested_capabilities}"
            )
            ...
            if session is not None:
                ...
                return await self._handle_with_session(
                    task, progress_callback, session, part_id, preferred_supplier, requested_capabilities, force_refresh
                )
            else:
    ...
    async def _handle_with_session(
        ...
        try:
            ...
            capabilities = self._determine_capabilities(supplier, supplier_config, requested_capabilities)
            ...
            if hasattr(client, "supports_scraping") and client.supports_scraping():
                ...
                if product_url or supplier_url:
                    ...
                    part_identifier = self._get_supplier_part_number_from_cached_data(
                        part_number, supplier_part_number_field
                    )
            ...
            logger.info(
                f"Starting unified enrichment for part {part_name} using {supplier} with identifier: {part_identifier}"
            )
            if progress_callback:
            ...
            if not engine_result["success"]:
                ...
                part_search_result = PartSearchResult(
                    ...
                    pricing=standardized_data.get("additional_properties", {})
                    .get("pricing_tiers_for_history", {})
                    .get("tiers"),
            ...
            logger.info(
                f"Enrichment completed: {len(enrichment_result.enriched_fields)} succeeded, {len(enrichment_result.failed_fields)} failed"
            )
            ...
            try:
                ...
                await activity_service.log_activity(
                    ...
                    user=user,
                    details={"supplier": supplier if supplier else "Unknown", "error": str(e)},
                )
            except Exception as log_error:
    ...
    def _determine_supplier_from_cached_data(
        self, part_supplier: Optional[str], part_vendor: Optional[str], preferred_supplier: Optional[str]
    ) -> str:
    ...
    def _determine_capabilities(
        self, supplier: str, supplier_config: Any, requested_capabilities: List[str]
    ) -> List[str]:
    ...
    def _get_supplier_part_number_from_cached_data(
        self, part_number: Optional[str], supplier_part_number: Optional[str]
    ) -> str:
            ...
            for field, cap_name in field_to_capability_map.items():
                if field in enrichment_result.enriched_fields:
                    ...
                    if value is not None:
                        ...
                        enrichment_results[cap_name] = {"success": True, field: value}
    ...
    async def _apply_enrichment_to_part(
        self, part: PartModel, enrichment_results: Dict[str, Any], supplier: str, task: TaskModel
    ) -> None:
        ...
        part_search_result = self._convert_enrichment_to_part_search_result(part, enrichment_results, supplier)
        ...
        if part_search_result:
            ...
            standardized_data = self.data_mapper.supplier_data_mapper.map_supplier_result_to_part_data(
                part_search_result, supplier, list(enrichment_results.keys())
            )
    ...
    def _convert_enrichment_to_part_search_result(
        ...
        try:
            ...
            result = PartSearchResult(
                ...
                or additional_data.get(f"{supplier_name.lower()}_part_number", "")
                or part.part_number
                or "",
    ...
    async def _apply_standardized_data_to_part(
        self, part: PartModel, standardized_data: Dict[str, Any], supplier: str, task: TaskModel
    ) -> None:
        ...
        try:
            ...
            logger.info(
                f"[APPLY DEBUG] Standardized data has supplier_part_number = '{standardized_data.get('supplier_part_number')}'"
            )
            ...
            additional_fields = [
                ...
                "data_quality_score",
                "rohs_status",
                "lifecycle_status",  # Moved from model_fields to additional_properties
            ]
            ...
            if isinstance(additional_payload, dict):
                ...
                payload_updates = self._merge_additional_properties_dict(
                    part.additional_properties, flattened_payload, path="additional_properties"
                )
            ...
            datasheet_url = standardized_data.get("additional_properties", {}).get(
                "datasheet_url"
            ) or part.additional_properties.get("datasheet_url")
    ...
    def _merge_additional_properties_dict(
        self, current: Dict[str, Any], updates: Dict[str, Any], path: str = ""
    ) -> List[str]:
        ...
        for key, value in updates.items():
            ...
            if isinstance(value, dict):
                ...
                updated_fields.extend(self._merge_additional_properties_dict(existing, value, full_path))
            else:
    ...
    async def _save_part_to_database(self, part: PartModel) -> None:
        ...
        with self.get_session() as session:
            try:
                ...
                if fresh_part:
                    logger.info(
                        f"[SAVE DEBUG] Fresh part from DB: supplier_part_number = '{fresh_part.supplier_part_number}'"
                    )
                    ...
                    if (
                        ...
                        and part.manufacturer_part_number != fresh_part.manufacturer_part_number
                    ):
                        logger.info(
                            f"Updating part manufacturer_part_number: '{fresh_part.manufacturer_part_number}' -> '{part.manufacturer_part_number}'"
                        )
                    ...
                    if part.supplier_part_number is not None:
                        logger.info(
                            f"[SAVE DEBUG] Updating part supplier_part_number: '{fresh_part.supplier_part_number}' -> '{part.supplier_part_number}'"
                        )
    ...
    async def _create_datasheet_download_task(
        self, part: PartModel, datasheet_url: str, supplier: str, parent_task: TaskModel
    ) -> None:
        """
