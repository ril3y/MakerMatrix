...
class BulkEnrichmentService(BaseService):
    ...
    async def _handle_bulk_enrichment_specific(
        ...
        try:
            ...
            logger.info(f"📊 [BULK ENRICHMENT] Final results: {len(successful_enrichments)} successful, {len(failed_enrichments)} failed")
        ...
        try:
            ...
            with self.get_session() as session:
                if supplier_filter:
                    ...
                    logger.info(f"📊 [BULK ENRICHMENT] Total parts with supplier filter '{supplier_filter}': {total_parts}")
                else:
            ...
            logger.info(f"📄 [BULK ENRICHMENT] Processing {total_parts} parts across {total_pages} pages (page size: {page_size})")
            ...
            for page_num in range(total_pages):
                ...
                for batch_start in range(0, len(parts), batch_size):
                    ...
                    logger.info(f"🔄 [BULK ENRICHMENT] Page {page_num + 1}, Batch {batch_number}/{page_batches}: {len(batch_parts)} parts")
                    ...
                    # Execute batch in parallel
                    logger.info(f"⚡ [BULK ENRICHMENT] Executing page {page_num + 1}, batch {batch_number} with {len(batch_tasks)} tasks")
                    batch_results = await asyncio.gather(*batch_tasks, return_exceptions=True)
                ...
                logger.info(f"📊 [BULK ENRICHMENT] Page {page_num + 1} completed: {len(page_successful)} successful, {len(page_failed)} failed")
