...
class SupplierIntegrationService:
        ...
        try:
            ...
            for capability in all_capabilities:
                if supplier.is_capability_available(capability):
                    ...
                    logger.debug(f"Capability {capability.value} not available for {supplier_name}: missing {missing_creds}")
    ...
    def validate_supplier_capabilities(self, supplier_name: str, capabilities: List[SupplierCapability]) -> List[SupplierCapability]:
        """
    ...
    async def execute_supplier_enrichment(
        ...
        try:
            ...
            logger.info(f"Enriching part {part.id} with supplier {supplier_name}, capabilities: {[cap.value for cap in valid_capabilities]}")
    ...
    def _convert_enrichment_result(self, enrichment_result: EnrichmentResult, supplier_name: str) -> Dict[str, Any]:
        ...
        if enrichment_result.data:
            ...
            if 'part_details' in enrichment_result.enriched_fields or 'get_part_details' in enrichment_result.enriched_fields:
