...
class DatasheetHandlerService(BaseService):
    ...
    async def handle_datasheet_fetch(
        ...
        try:
            ...
            if part and result.success and result.datasheet_url:
                await self._process_datasheet_result(part, result, supplier, part_number, progress_callback)
    ...
    async def _process_datasheet_result(
        ...
        download_result = file_download_service.download_datasheet(
            url=result.datasheet_url, part_number=part_number, supplier=supplier
        )
        ...
        await self._create_or_update_datasheet_record(part, result, supplier, part_number, download_result)
    ...
    async def _create_or_update_datasheet_record(
        self, part: PartModel, result: Any, supplier: str, part_number: str, download_result: Optional[Dict[str, Any]]
    ) -> None:
        """
            ...
            if not existing_datasheet:
                ...
                if download_result:
                    ...
                    self.datasheet_repository.mark_download_failed(
                        session, existing_datasheet.id, "Failed to download datasheet file"
                    )
