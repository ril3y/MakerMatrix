...
from MakerMatrix.models.models import (
    PartModel, CategoryModel, LocationModel, PartCategoryLink, PartOrderSummary
)
...
class AnalyticsService:
    ...
    def get_part_order_frequency(
        ...
        with Session(self.engine) as session:
            results = session.query(
                ...
                PartModel.quantity,
                PartOrderSummary.total_orders,
                PartOrderSummary.average_price,
                PartOrderSummary.last_ordered_date
            ).join(
                PartOrderSummary, PartModel.id == PartOrderSummary.part_id
            ).filter(
                PartOrderSummary.total_orders >= min_orders
            ).order_by(
                PartOrderSummary.total_orders.desc()
            ).limit(limit).all()
            ...
            return [
                {
                    ...
                    'last_order_date': result.last_ordered_date.isoformat() if result.last_ordered_date else None
                }
                for result in results
            ]
            ...
            return [
                {
                    ...
                    'part_number': result.part_number,
                    'current_quantity': result.quantity,
                    'minimum_quantity': None,  # Field not available in current model
    ...
    def get_inventory_value(self) -> Dict[str, Any]:
        ...
        with Session(self.engine) as session:
            ...
            results = session.query(
                func.sum(PartModel.quantity * PartOrderSummary.average_price).label('total_value'),
                func.count(PartModel.id).label('total_parts'),
                func.sum(PartModel.quantity).label('total_units')
            ).join(
                PartOrderSummary, PartModel.id == PartOrderSummary.part_id
            ).filter(
                PartOrderSummary.average_price.isnot(None)
            ).first()
            ...
            unpriced_count = session.query(
                ...
                or_(
                    PartOrderSummary.average_price.is_(None),
                    PartOrderSummary.part_id.is_(None)
                )
            ).scalar()
