...
const ImportSelector: React.FC<ImportSelectorProps> = ({ onImportComplete }) => {
  ...
  const selectedParserInfo = parsers.find(
    (p) => p.id === selectedParser || p.name === selectedParser
  )
  ...
  useEffect(() => {
    if (selectedParser && selectedParserInfo && selectedParserInfo.enrichment_available) {
      ...
      if (selectedParser.toLowerCase() === 'digikey') {
        ...
        defaultCapabilities = capabilities.filter(
          (cap) =>
            cap === 'get_part_details' || cap === 'fetch_datasheet' || cap === 'fetch_pricing_stock'
        )
      } else {
        // For other suppliers, select the most common capabilities
        defaultCapabilities = capabilities.filter(
          (cap) => cap === 'get_part_details' || cap === 'fetch_datasheet'
        )
      }
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-secondary">
          Upload your order files (CSV or XLS) below. We'll automatically detect the supplier and
          format.
        </p>
      </div>
          ...
          <div className="card p-6">
            ...
            {uploadedFile && (
              <div className="mt-6 space-y-4">
                    ...
                    <div className="flex items-start">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-blue-700 dark:text-blue-300 mb-3">
                          {selectedParserInfo.name} can automatically enrich imported parts with
                          additional data like datasheets, images, and pricing information.
                        </p>
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <button
                                onClick={() =>
                                  setSelectedEnrichmentCapabilities(
                                    selectedParserInfo.enrichment_capabilities || []
                                  )
                                }
                            ...
                            {selectedParserInfo.enrichment_capabilities.map((capability) => (
                              <label
                                key={capability}
                                className="flex items-center hover:bg-blue-100 dark:hover:bg-blue-900/30 p-2 rounded transition-colors"
                              >
                                <input
                                  type="checkbox"
                                  ...
                                  onChange={(e) => {
                                    if (e.target.checked) {
                                      setSelectedEnrichmentCapabilities((prev) => [
                                        ...prev,
                                        capability,
                                      ])
                                    } else {
                                ...
                                <div className="flex items-center space-x-2 flex-1">
                                  ...
                                  {selectedParser.toLowerCase() === 'digikey' &&
                                    (capability === 'get_part_details' ||
                                      capability === 'fetch_datasheet' ||
                                      capability === 'fetch_pricing_stock') && (
                            ...
                            {selectedEnrichmentCapabilities.length > 0 && (
                              <div className="mt-3 p-2 bg-green-100 dark:bg-green-900/30 rounded text-sm">
                                <p className="text-green-800 dark:text-green-200">
                                  <strong>Selected:</strong> {selectedEnrichmentCapabilities.length}{' '}
                                  capability(ies) will be applied after import.
                                </p>
                              </div>
                            )}
                          </div>
                        ...
                        {selectedParserInfo.enrichment_missing_credentials?.length > 0 && (
                          <div className="mt-3 p-2 bg-yellow-100 dark:bg-yellow-900/30 rounded text-sm">
                            <p className="text-yellow-800 dark:text-yellow-200">
                              <strong>Note:</strong> Some enrichment features may require additional
                              configuration:{' '}
                              {selectedParserInfo.enrichment_missing_credentials.join(', ')}
                            </p>
                          </div>
                        )}
                      </div>
                ...
                {selectedParserInfo &&
                  !selectedParserInfo.enrichment_available &&
                  selectedParserInfo.enrichment_missing_credentials?.length > 0 && (
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-red-700 dark:text-red-300 mb-3">
                            {selectedParserInfo.name} enrichment requires additional configuration
                            to access detailed part information, datasheets, and pricing.
                          </p>
                          ...
                          <div className="mt-3 p-2 bg-blue-100 dark:bg-blue-900/40 rounded text-sm">
                            <p className="text-blue-800 dark:text-blue-200">
                              <strong>Import will still work</strong> - you'll get basic part
                              information from the file, but no additional enrichment data like
                              datasheets or real-time pricing.
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="bg-background-secondary rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <span
                          className={`ml-2 font-medium ${filePreview.is_supported ? 'text-success' : 'text-error'}`}
                        >
                          {filePreview.is_supported
                            ? 'Supported'
                            : filePreview.validation_errors?.some((err) =>
                                  err.includes('Backend not connected')
                                )
                              ? 'Backend Offline'
                              : 'Not Supported'}
                        </span>
