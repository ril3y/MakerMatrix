...
const LCSCImporter: React.FC<LCSCImporterProps> = ({ onImportComplete }) => {
  const validateFile = (file: File): boolean => {
    ...
    if (!fileName.endsWith('.csv')) {
      toast.error('LCSC files should be in CSV format.')
      return false
    }
      ...
      <div className="text-center space-y-4">
        <div>
          ...
          <p className="text-secondary mb-4">
            Upload LCSC order CSV files to automatically add parts to your inventory
          </p>
        </div>
      </div>
