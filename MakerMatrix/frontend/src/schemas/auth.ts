...
export const loginSchema = z.object({
  username: commonValidation
    ...
    .max(
      ...
      `Username must be no more than ${fieldConstraints.username.maxLength} characters`
    ),
  ...
  password: commonValidation
    ...
    .max(
      fieldConstraints.password.maxLength,
      `Password must be no more than ${fieldConstraints.password.maxLength} characters`
    ),
})
...
export const twoFactorSetupSchema = z.object({
  ...
  code: commonValidation
    .requiredString('Verification code')
    .length(6, 'Verification code must be 6 digits')
    .refine((val) => /^\d{6}$/.test(val), 'Verification code must be 6 digits'),
})
...
export const passwordStrengthRules = {
  ...
  commonPasswords: ['password', '123456', 'password123', 'admin', 'qwerty', 'letmein'],
}
