...
export const basePartSchema = z.object({
  part_name: commonValidation.requiredString('Part name')
    .max(fieldConstraints.name.maxLength, `Part name must be no more than ${fieldConstraints.name.maxLength} characters`),
  
  part_number: commonValidation.optionalTrimmedString
    .refine(
...
export const createPartSchema = basePartSchema.extend({
  ...
  part_name: commonValidation.requiredString('Part name')
    .min(fieldConstraints.name.minLength, `Part name must be at least ${fieldConstraints.name.minLength} character`)
    .max(fieldConstraints.name.maxLength, `Part name must be no more than ${fieldConstraints.name.maxLength} characters`),
})
...
export const additionalPropertySchema = z.object({
  key: commonValidation.requiredString('Property name')
