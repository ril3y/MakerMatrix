...
export const baseCategorySchema = z.object({
  name: commonValidation.requiredString('Category name')
    .min(fieldConstraints.name.minLength, `Category name must be at least ${fieldConstraints.name.minLength} character`)
    .max(fieldConstraints.name.maxLength, `Category name must be no more than ${fieldConstraints.name.maxLength} characters`)
    .refine(
...
export const mergeCategoriesSchema = z.object({
  source_category_ids: z.array(commonValidation.uuid).min(1, 'At least one source category must be selected'),
