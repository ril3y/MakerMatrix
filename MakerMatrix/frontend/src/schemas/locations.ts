...
export const baseLocationSchema = z.object({
  name: commonValidation
    .requiredString('Location name')
    .min(
      fieldConstraints.name.minLength,
      `Location name must be at least ${fieldConstraints.name.minLength} character`
    )
    .max(
...
export const locationHierarchySchema = z
  .object({
...
export const locationValidationRules = {
  ...
  maxHierarchyDepth: (currentDepth: number, maxDepth = 10) => {
    return z
      .string()
      .refine(() => currentDepth < maxDepth, `Location hierarchy cannot exceed ${maxDepth} levels`)
  },
