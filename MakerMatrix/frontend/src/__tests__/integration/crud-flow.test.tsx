...
describe('CRUD Flow Integration Tests', () => {
  ...
  describe('Parts CRUD Flow', () => {
    it('should complete full CRUD cycle for parts', async () => {
      ...
      // Verify update
      await waitFor(() => {
        expect(partService.updatePart).toHaveBeenCalledWith('test-id', expect.objectContaining({
          quantity: 150
        }))
      })
    ...
    it('should handle API errors gracefully', async () => {
      ...
      // Mock error response
      vi.mocked(partService.getAllParts).mockRejectedValueOnce(
        new Error('Failed to fetch parts')
      )
  ...
  describe('Cross-Entity Integration', () => {
    it('should update part location and categories', async () => {
      ...
      await waitFor(() => {
        expect(partService.updatePart).toHaveBeenCalledWith('part-1', expect.objectContaining({
          location_id: 'loc-2',
