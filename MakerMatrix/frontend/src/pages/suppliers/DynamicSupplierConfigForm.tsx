...
export const DynamicSupplierConfigForm: React.FC<DynamicSupplierConfigFormProps> = ({
  ...
  const loadSupplierData = async () => {
    try {
      ...
      setLoadingError(`Failed to load supplier configuration: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        ...
        <span className="ml-3 text-gray-600 dark:text-gray-400">Loading supplier configuration...</span>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {credentialFields.map((field) => (
              <div key={field.name}>
                ...
                {renderField(field, credentials[field.name], (value) => handleCredentialChange(field.name, value))}
                {field.help_text && (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <p className="mt-1 text-sm text-yellow-700 dark:text-yellow-300">
                This supplier uses OAuth authentication. After configuring your credentials,
                you'll need to complete the OAuth authorization process to use this supplier.
              </p>
            </div>
          </div>
        </div>
