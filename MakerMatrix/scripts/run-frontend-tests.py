...
class TestRunner:
    ...
    def setup_environment(self):
        ...
        if not venv_path.exists():
            ...
            subprocess.run([sys.executable, "-m", "venv", "venv_test"], cwd=self.project_root, check=True)

            # Install Python dependencies
            pip_path = venv_path / "bin" / "pip"
            subprocess.run([str(pip_path), "install", "--upgrade", "pip"], check=True)
            subprocess.run([str(pip_path), "install", "-r", "requirements.txt"], cwd=self.project_root, check=True)
    ...
    def start_backend(self, timeout: int = 30) -> bool:
        ...
        self.backend_process = subprocess.Popen(
            [str(venv_python), "-m", "uvicorn", "MakerMatrix.main:app", "--host", "0.0.0.0", "--port", "57891"],
    ...
    def start_frontend(self, mode: str = "dev", timeout: int = 30) -> bool:
        ...
        self.frontend_process = subprocess.Popen(
            cmd, cwd=self.frontend_path, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
    ...
    def run_accessibility_tests(self) -> bool:
        ...
        try:
            with self.servers_running():
                ...
                self.run_command(["npx", "playwright", "test", "--grep", "accessibility"], cwd=self.frontend_path)
