...
class MouserSupplier(BaseSupplier):
    ...
    def get_capability_requirements(self) -> Dict[SupplierCapability, CapabilityRequirement]:
        ...
        for capability in self.get_capabilities():
            if capability == SupplierCapability.IMPORT_ORDERS:
                ...
                requirements[capability] = CapabilityRequirement(
                    capability=capability, required_credentials=api_key_req
                )
    ...
    def _get_http_client(self):
        ...
        if not hasattr(self, "_http_client") or not self._http_client:
            ...
            retry_config = RetryConfig(
                max_retries=3, base_delay=delay_seconds, max_delay=30.0, retry_on_status=[429, 500, 502, 503, 504]
            )
    ...
    async def authenticate(self) -> bool:
        ...
        if not api_key:
            raise SupplierConfigurationError(
                "Mouser requires an API key", supplier_name="mouser", details={"missing_credentials": ["api_key"]}
            )
        ...
        try:
            ...
            search_data = {"SearchByKeywordRequest": {"keyword": "test", "records": 1, "startingRecord": 0}}
    ...
    async def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            search_data = {"SearchByKeywordRequest": {"keyword": "resistor", "records": 1, "startingRecord": 0}}

            response = await http_client.post(
                url, endpoint_type="test_connection", params=params, json_data=search_data
            )
            ...
            return {
                ...
                "details": {"exception": str(e), "traceback": traceback.format_exc(), "unexpected_error": True},
            }
        ...
        try:
            ...
            if response.success:
                ...
                raise SupplierConnectionError(
                    f"Search failed: {response.status} - {response.error_message}", supplier_name="mouser"
                )
        except SupplierError:
            # Re-raise supplier errors as-is
            raise
        except Exception as e:
            raise SupplierConnectionError(f"Network error during search: {str(e)}", supplier_name="mouser")
    ...
    def _parse_search_results(self, data: Dict[str, Any]) -> List[PartSearchResult]:
        ...
        for part in parts:
            ...
            additional_data = {
                ...
                # Physical properties
                "unit_weight_kg": (
                    part.get("UnitWeightKg", {}).get("UnitWeight")
                    if isinstance(part.get("UnitWeightKg"), dict)
                    else None
                ),
                # Simulation tools
            ...
            additional_data = {
                k: v for k, v in additional_data.items() if v is not None and v != "" and v != [] and v != False
            }
            ...
            product_detail_url = part.get("ProductDetailUrl", "")
            logger.info(
                f"[MOUSER DEBUG] Part {part.get('MouserPartNumber', 'unknown')}: DataSheetUrl='{datasheet_url}', ProductDetailUrl='{product_detail_url}'"
            )
    ...
    async def get_part_details(self, supplier_part_number: str) -> Optional[PartSearchResult]:
        ...
        try:
            response = await http_client.post(
                url, endpoint_type="get_part_details", params=params, json_data=search_data
            )
    ...
    async def fetch_datasheet(self, supplier_part_number: str) -> Optional[str]:
        ...
        async def _impl():
            ...
            if part_details.datasheet_url:
                logger.info(
                    f"Mouser API provided datasheet URL for {supplier_part_number}: {part_details.datasheet_url}"
                )
            ...
            try:
                ...
                if not df.empty:
                    ...
                    mouser_specific_indicators = [
                        ...
                        "mouser",
                        "mfr. #",
                        "mfr #",  # Added more flexible patterns
                    ]
                    generic_indicators = [
                        ...
                        "order qty",
                        "qty",
                        "desc",  # Added more flexible patterns
                    ]
    ...
    async def import_order_file(self, file_content: bytes, file_type: str, filename: str = None) -> ImportResult:
        ...
        try:
            ...
            if not parts:
                return ImportResult(success=False, error_message="No valid parts found in Excel file", warnings=errors)
    ...
    def _build_mouser_additional_properties(
