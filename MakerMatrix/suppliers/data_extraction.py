...
class DataExtractor:
    ...
    def extract_pricing(
        ...
        for path in pricing_paths:
            ...
            for i, break_data in enumerate(pricing_data):
                ...
                # Extract quantity
                quantity = self.safe_cast(self.safe_get(break_data, quantity_key, 1), int, 1)

                # Extract price
                price = self.safe_cast(self.safe_get(break_data, price_key, 0.0), float, 0.0)
                ...
                pricing_breaks.append(PricingBreak(quantity=quantity, price=price, currency=currency.upper()))
        ...
        return ExtractionResult.error_result(f"No valid pricing data found in paths: {pricing_paths}", warnings)

    def extract_single_price(
        self, data: Dict[str, Any], price_paths: List[str], quantity: int = 1, currency: str = "USD"
    ) -> ExtractionResult:
        """
        ...
        for path in price_paths:
            ...
            if price is not None:
                return ExtractionResult.success_result(
                    [PricingBreak(quantity=quantity, price=price, currency=currency)], warnings
                )
            else:
                warnings.append(f"Could not convert price value '{price_value}' to float at path '{path}'")

        return ExtractionResult.error_result(f"No valid price found in paths: {price_paths}", warnings)
    ...
    def extract_url(
        ...
        return ExtractionResult.error_result(f"No valid URL found in paths: {url_paths}", warnings)

    def extract_image_url(
        self, data: Dict[str, Any], image_paths: List[str], base_url: Optional[str] = None
    ) -> ExtractionResult:
        """Extract and validate image URL"""
        return self.extract_url(
            data, image_paths, base_url, required_extensions=[".jpg", ".jpeg", ".png", ".gif", ".webp"]
        )

    def extract_datasheet_url(
        self, data: Dict[str, Any], datasheet_paths: List[str], base_url: Optional[str] = None
    ) -> ExtractionResult:
        """Extract and validate datasheet URL"""
        return self.extract_url(data, datasheet_paths, base_url, required_extensions=[".pdf", ".doc", ".docx"])

    # ========== Specification Extraction ==========

    def extract_specifications(
        self, data: Dict[str, Any], spec_mapping: Dict[str, Union[str, List[str]]], normalize_keys: bool = True
    ) -> ExtractionResult:
        """
    ...
    def extract_clean_text(
        self, data: Dict[str, Any], text_paths: List[str], max_length: Optional[int] = None, strip_html: bool = True
    ) -> ExtractionResult:
        """
        ...
        return ExtractionResult.error_result(f"No valid text found in paths: {text_paths}", warnings)

    # ========== Stock Quantity Extraction ==========

    def extract_stock_quantity(
        self, data: Dict[str, Any], stock_paths: List[str], availability_paths: Optional[List[str]] = None
    ) -> ExtractionResult:
        """
        ...
        if availability_paths:
            for path in availability_paths:
                ...
                if availability is not None:
                    ...
                    if any(
                        indicator in avail_str
                        for indicator in ["discontinued", "obsolete", "not available", "out of stock"]
                    ):
        ...
        return ExtractionResult.error_result(f"No valid stock quantity found in paths: {stock_paths}", warnings)
...
def extract_common_part_data(extractor: DataExtractor, data: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if "pricing_paths" in config:
        ...
        if pricing_result.success:
            extracted_data["pricing"] = [
                {"quantity": break_item.quantity, "price": break_item.price, "currency": break_item.currency}
