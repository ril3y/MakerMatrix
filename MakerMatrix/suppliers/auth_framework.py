...
class APIKeyAuthenticator(BaseAuthenticator):
    ...
    async def authenticate(self, credentials: Dict[str, Any]) -> AuthResult:
        ...
        self._current_token = AuthToken(access_token=api_key, token_type="ApiKey")
...
class OAuth2ClientCredentialsAuthenticator(BaseAuthenticator):
    ...
    def __init__(
        self, supplier_name: str, http_client: SupplierHTTPClient, token_url: str, scope: Optional[str] = None
    ):
        super().__init__(supplier_name, http_client)
    ...
    async def authenticate(self, credentials: Dict[str, Any]) -> AuthResult:
        ...
        if not client_id or not client_secret:
            return AuthResult(success=False, error_message="Client ID and client secret are required")
    ...
    async def refresh_token(self) -> AuthResult:
        ...
        if not self._client_id or not self._client_secret:
            return AuthResult(success=False, error_message="Client credentials not available for refresh")
    ...
    async def _request_token(self) -> AuthResult:
        ...
        try:
            ...
            response = await self.http_client.post(
                self.token_url, endpoint_type="oauth_token", headers=headers, data=data
            )
            ...
            if not access_token:
                return AuthResult(
                    success=False, error_message="No access token in response", status_code=response.status
                )
            ...
            return AuthResult(
                ...
                additional_data={"expires_in": expires_in, "scope": self._current_token.scope},
            )
...
class BearerTokenAuthenticator(BaseAuthenticator):
    ...
    async def authenticate(self, credentials: Dict[str, Any]) -> AuthResult:
        ...
        if not bearer_token:
            return AuthResult(success=False, error_message="Bearer token is required")

        self._current_token = AuthToken(access_token=bearer_token, token_type="Bearer")
...
class AuthenticationManager:
    ...
    def register_oauth2_client_credentials(
        self, token_url: str, scope: Optional[str] = None
    ) -> OAuth2ClientCredentialsAuthenticator:
        """Register OAuth2 client credentials authentication method"""
        auth = OAuth2ClientCredentialsAuthenticator(self.supplier_name, self.http_client, token_url, scope)
    ...
    async def authenticate(self, auth_method: str, credentials: Dict[str, Any]) -> AuthResult:
        ...
        if auth_method not in self._authenticators:
            return AuthResult(success=False, error_message=f"Unknown authentication method: {auth_method}")
...
async def test_connection_with_auth(
    ...
    try:
        ...
        # Make test request
        response = await http_client.get(test_url, endpoint_type="test_connection", headers=auth_headers)

        if not response.success:
            return {
                "success": False,
                "message": f"Connection test failed with status {response.status}",
                "details": {"status_code": response.status, "error": response.error_message, "url": test_url},
            }

        # Check for expected indicators in response
        if expected_indicators:
            response_text = json.dumps(response.data).lower()
            found_indicators = [indicator for indicator in expected_indicators if indicator.lower() in response_text]
    ...
    **kwargs,
    ...
    if not await auth_manager.ensure_authenticated():
        ...
        raise SupplierAuthenticationError(
            f"Authentication required for {http_client.supplier_name}", supplier_name=http_client.supplier_name
        )
