...
class DigiKeySupplier(BaseSupplier):
    ...
    def get_supplier_info(self) -> SupplierInfo:
        return SupplierInfo(
            ...
            supported_file_types=[
                ...
                "xls",
                "xlsx",
            ],  # DigiKey exports in CSV format but files may have Excel extensions
        )
    ...
    def _get_http_client(self):
        ...
        if not hasattr(self, "_http_client") or not self._http_client:
            ...
            retry_config = RetryConfig(
                max_retries=3, base_delay=delay_seconds, max_delay=60.0, retry_on_status=[429, 500, 502, 503, 504]
            )
    ...
    async def authenticate(self) -> bool:
        ...
        if self._has_valid_cached_token():
            logger.debug(
                "DigiKey: Using cached access token (valid for %s more seconds)",
        ...
        if not self.is_configured():
            raise SupplierConfigurationError(
                "DigiKey supplier not configured. Please provide client_id and client_secret.", supplier_name="digikey"
            )
        ...
        if not client_id or not client_secret:
            raise SupplierConfigurationError(
                "DigiKey requires both client_id and client_secret", supplier_name="digikey"
            )
        ...
        try:
            ...
            raise SupplierAuthenticationError(f"DigiKey authentication failed: {str(e)}", supplier_name="digikey")
    ...
    async def _authenticate_production(self) -> bool:
        ...
        try:
            if not DIGIKEY_API_AVAILABLE:
                raise SupplierConfigurationError(
                    "DigiKey API library not available. Install with: pip install digikey-api", supplier_name="digikey"
                )
            ...
            if oauth_callback_url and not oauth_callback_url.startswith(("http://", "https://")):
                raise SupplierConfigurationError(
                    "oauth_callback_url must be a valid URL starting with http:// or https://", supplier_name="digikey"
                )
            ...
            try:
                ...
                raise SupplierConfigurationError(
                    f"Cannot create token storage directory: {storage_dir}", supplier_name="digikey"
                )
            ...
            raise SupplierAuthenticationError(f"DigiKey authentication setup failed: {str(e)}", supplier_name="digikey")
    ...
    async def _refresh_access_token(self) -> bool:
        ...
        try:
            ...
            response = await http_client.post(
                self._get_token_url(), endpoint_type="refresh_token", headers=headers, data=data
            )
    ...
    def get_oauth_authorization_url(self) -> str:
        ...
        params = {"response_type": "code", "client_id": client_id, "redirect_uri": callback_url, "scope": "read"}
    ...
    async def exchange_code_for_tokens(self, authorization_code: str) -> bool:
        ...
        try:
            http_client = self._get_http_client()
            response = await http_client.post(
                self._get_token_url(), endpoint_type="exchange_code", headers=headers, data=data
            )
            ...
            if response.success:
                ...
                raise SupplierAuthenticationError(f"Token exchange failed: {error_data}", supplier_name="digikey")
        except SupplierError:
            # Re-raise supplier errors as-is
            raise
        except Exception as e:
            raise SupplierConnectionError(f"Connection error during token exchange: {str(e)}", supplier_name="digikey")
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "details": {"error": str(config_error), "configuration_needed": True},
                }
            ...
            return {
                ...
                "details": {"exception": str(e), "unexpected_error": True},
            }
    ...
    async def search_parts(self, query: str, limit: int = 50) -> List[PartSearchResult]:
        ...
        try:
            ...
            search_body = {
                ...
                "Sort": {"SortOption": "SortByQuantityAvailable", "Direction": "Descending"},
            }

            http_client = self._get_http_client()
            response = await http_client.post(
                search_url, endpoint_type="search_parts", headers=headers, json_data=search_body
            )
    ...
    async def get_part_details(self, supplier_part_number: str) -> Optional[PartSearchResult]:
        ...
        async def _impl():
            ...
            if is_likely_mpn:
                logger.info(
                    f"🔍 Input '{input_part_number}' appears to be a manufacturer part number. Searching first..."
                )
                try:
            ...
            try:
                ...
                if response.success:
                    ...
                    if product:
                        logger.info(
                            f"✅ DigiKey retrieved part data for {part_number_to_query}: {product.get('ManufacturerProductNumber', 'Unknown MPN')}"
                        )
                    else:
                    ...
                    if product:
                        ...
                        additional_data = {
                            ...
                            "base_product_number": (
                                product.get("BaseProductNumber", {}).get("Name", "")
                                if product.get("BaseProductNumber")
                                else ""
                            ),
                            ...
                            "manufacturer_lead_weeks": (
                                int(product.get("ManufacturerLeadWeeks", 0))
                                if product.get("ManufacturerLeadWeeks")
                                else 0
                            ),
                    ...
                    if response.status == 401 and "not subscribed to this API" in error_text:
                        ...
                        raise SupplierAuthenticationError(
                            f"DigiKey authentication failed: {error_text}", supplier_name="digikey"
                        )
                    else:
                        raise SupplierConnectionError(
                            f"DigiKey API error ({response.status}): {error_text}", supplier_name="digikey"
                        )
        ...
        if file_content:
            try:
                ...
                if filename_lower.endswith((".xls", ".xlsx")):
                    try:
                        ...
                        if not df.empty:
                            ...
                            generic_indicators = ["manufacturer part number", "part number", "quantity"]

                            # Require at least one DigiKey-specific indicator
                            has_digikey_specific = any(
                                indicator in headers for indicator in digikey_specific_indicators
                            )
                ...
                # For CSV files, check text content
                    ...
                    generic_indicators = ["manufacturer part number", "part number", "quantity", "mfr part"]

                    # Require at least one DigiKey-specific indicator
                    has_digikey_specific = any(
                        indicator in content_normalized for indicator in digikey_specific_indicators
                    )
        ...
        try:
            ...
            if file_type_lower in ["xls", "xlsx"]:
                try:
                    ...
                    try:
                        df = pd.read_excel(
                            io.BytesIO(file_content), engine="openpyxl" if file_type_lower == "xlsx" else None
                        )
                        ...
                        try:
                            ...
                            if "," in content_str and "\n" in content_str:
                                logger.warning(
                                    f"File {filename} has .{file_type_lower} extension but appears to be CSV format, processing as CSV"
                                )
                            else:
            ...
            if df.empty:
                return ImportResult(success=False, error_message="File contains no data rows")
            ...
            if not column_mapper.validate_required_columns(mapped_columns, required_fields):
                return ImportResult(
                    ...
                    warnings=[
                        f"Missing required fields: {[field for field in required_fields if field not in mapped_columns]}"
                    ],
                )
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    if extracted_data.get("unit_price"):
                        try:
                            unit_price = float(
                                str(extracted_data["unit_price"]).replace("$", "").replace(",", "").replace('"', "")
                            )
                        except (ValueError, TypeError):
                            pass
                    ...
                    if extracted_data.get("order_price"):
                        try:
                            order_price = float(
                                str(extracted_data["order_price"]).replace("$", "").replace(",", "").replace('"', "")
                            )
                        except (ValueError, TypeError):
                            pass
                    ...
                    part_search_result = PartSearchResult(
                        ...
                        description=(
                            extracted_data.get("description", "").strip() if extracted_data.get("description") else None
                        ),
            ...
            if not parts:
                return ImportResult(success=False, error_message="No valid parts found in file", warnings=errors)
            ...
            return ImportResult(
                success=False, error_message=f"Error importing DigiKey CSV: {str(e)}", warnings=[traceback.format_exc()]
            )

    def _build_digikey_additional_properties(
