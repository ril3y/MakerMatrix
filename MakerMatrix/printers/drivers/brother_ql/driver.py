...
class BrotherQLModern(BasePrinter):
    ...
    def __init__(self, printer_id: str, name: str, model: str, backend: str, 
                 identifier: str, dpi: int = 300, scaling_factor: float = 1.0,
        ...
        try:
            ...
            if success:
                return PrintJobResult(
                    success=True,
                    job_id=job_id,
                    message=f"Printed {copies} label(s) on {label_size}mm Brother QL"
                )
            else:
                return PrintJobResult(
                    success=False,
                    job_id=job_id,
                    error=error_msg or "Unknown print error"
                )
    ...
    async def preview_label(self, image: Image.Image, label_size: str) -> PreviewResult:
        ...
        draw.rectangle([border_width-1, border_width+header_height-1, 
                       preview_width-border_width, preview_height-border_width], 
        ...
        draw.rectangle([border_width, border_width, preview_width-border_width, border_width+header_height], 
                      fill='#00458B')
        ...
        try:
            if self.backend == "network":
                ...
                return TestResult(
                    ...
                    message=f"Connected to Brother QL at {host}:{port}"
                )
                ...
                return TestResult(
                    success=True,
                    response_time_ms=response_time,
                    message=f"USB Brother QL connection assumed available"
                )
