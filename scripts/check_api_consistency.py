...
class APIConsistencyChecker(ast.NodeVisitor):
    ...
    def check_route_function(self, node: ast.FunctionDef):
        ...
        allowed_exceptions = {
            'get_image', 'serve_index_html', 'upload_image', 
            'get_current_user', 'serve_frontend', 'websocket_endpoint',
            'download_backup', 'export_data'
        }
    ...
    def get_call_name(self, node: ast.Call) -> str:
        ...
        if isinstance(node.func, ast.Name):
            ...
            return f"{self.get_call_name(node.func.value) if hasattr(node.func.value, 'id') else '...'}.{node.func.attr}"
        else:
            return "unknown"
