...
def print_vulnerability(severity: str, title: str, description: str, details: Dict = None):
    ...
    test_results[severity.lower()].append({
        "title": title,
...
def test_authorization():
    ...
    status, response = make_request("POST", "/api/tasks/quick/database_backup",
                                   headers=headers, data=backup_request)
...
def test_parameter_injection():
    ...
    for payload in path_traversal_payloads:
        ...
        status, response = make_request("POST", "/api/tasks/quick/part_enrichment",
                                       headers=headers, data=part_request)
    ...
    status, response = make_request("POST", "/api/tasks/quick/part_enrichment",
                                   headers=headers, data=injection_request)
...
def test_access_control():
    ...
    status, response = make_request("POST", "/api/tasks/quick/part_enrichment",
                                   headers=headers, data=test_task_request)
    ...
    status, response = make_request("POST", "/api/tasks/quick/part_enrichment",
                                   headers=headers, data=modify_request)
...
def test_rate_limiting():
    ...
    for i in range(15):
        task_request = {
            "part_id": f"rate-limit-test-{i}",
            "supplier": "digikey",
            "capabilities": ["fetch_datasheet"]
        }
        status, response = make_request("POST", "/api/tasks/quick/part_enrichment",
                                       headers=headers, data=task_request)
    ...
    status, response = make_request("POST", "/api/tasks/quick/bulk_enrichment",
                                   headers=headers, data=large_bulk_request)
...
def test_file_operations():
    ...
    status, response = make_request("POST", "/api/tasks/quick/file_import_enrichment",
                                   headers=headers, data=file_import_request)
    ...
    for url in ssrf_urls:
        datasheet_request = {
            "part_id": "test-part-ssrf",
            "datasheet_url": url,
            "supplier": "digikey"
        }
        status, response = make_request("POST", "/api/tasks/quick/datasheet_download",
                                       headers=headers, data=datasheet_request)

        if status == 200:
            print_vulnerability(
                "CRITICAL",
                "SSRF in Datasheet Download",
                f"SSRF-prone URL accepted: {url}",
                {"url": url}
            )
            break
    else:
...
def test_task_management():
    ...
    task_request = {
        "part_id": "test-management-security",
        "supplier": "digikey",
        "capabilities": ["fetch_datasheet"]
    }
    status, response = make_request("POST", "/api/tasks/quick/part_enrichment",
    ...
    status, response = make_request("POST", f"/api/tasks/{test_task_id}/cancel",
                                   headers=headers)
    if status == 200:
    ...
    status, response = make_request("PUT", f"/api/tasks/{test_task_id}",
                                   headers=headers, data=malicious_update)

    if status == 200:
        print_vulnerability(
            "MEDIUM",
            "Unrestricted Task Update",
            "Task can be updated with arbitrary data",
            {"response": response}
        )
    elif status == 403:
    ...
    status, response = make_request("DELETE", f"/api/tasks/{test_task_id}",
                                   headers=headers)
    if status == 200:
...
def generate_report():
    ...
    print(f"Total Vulnerabilities Found: {Colors.FAIL if total_vulnerabilities > 0 else Colors.OKGREEN}{total_vulnerabilities}{Colors.ENDC}")
